"""JSON-answer render for gate-contribution-factorial.

Ported (logic) from `placebo-seed-distribution-census/render.py` (itself
ported from RR3's own render.py), read in full before writing this. Prompt
contract, anchor position, and thinking-off self-check are unchanged; only
the environment variable names are namespaced to THIS experiment
(GATEFACT_RENDER_MODEL / GATEFACT_RENDER_REVISION) so this render cache can
never collide with any sibling experiment's own render module in the same
process. Family-agnostic: `steer_lib.load_model` sets these env vars to
whichever family/revision is currently loaded, so the SAME render.py serves
both factorial families, never simultaneously in one process.
"""
...
def assert_no_think_scaffolding(rendered_prompt: str, *, model: str) -> None:
    ...
    (enable_thinking=False pin, cell.yaml write.generation.enable_thinking)."""
...
def _log_fallback_once(model: str, mode: str, detail: str) -> None:
    ...
    print(f"[gatefact-render] {model}: {detail}", file=sys.stderr, flush=True)


def _tokenizer():
    global _TOKENIZER, _TOKENIZER_KEY
    model = os.environ.get("GATEFACT_RENDER_MODEL")
    if not model:
        raise RuntimeError("GATEFACT_RENDER_MODEL must name the HF tokenizer repo")
    revision = os.environ.get("GATEFACT_RENDER_REVISION") or None
    key = (model, revision)
...
def render(row: dict) -> str:
    ...
    tok = _tokenizer()
    model = os.environ.get("GATEFACT_RENDER_MODEL", "<unset>")
