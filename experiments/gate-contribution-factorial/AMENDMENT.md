...
Status: SIGNED 2026-07-15 (exploratory instrument/mechanism tier; reported
separately from every locked surface). All sign-time decision knobs are resolved
in the Decision record below; no result was known when any threshold was fixed.
...
Filled at resolve. Record per family: P1/P2/P3 and S1 verdicts with Wilson CIs on
every rate and bootstrap CIs on every gap; the fired counts per arm; the
per-arm confab and known rates; the gate-selectivity and cost-protection gaps;
the direction-specificity sign/percentile/ratio evaluation against the census
null; the SC0-SC3/CG1 gate results; the scoreboard adjudication; and the
one-sentence summary that also goes into `verdict:` in the manifest. Reaffirm that
no locked verdict moved.
