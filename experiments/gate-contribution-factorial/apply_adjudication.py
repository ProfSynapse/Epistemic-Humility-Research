#!/usr/bin/env python3
"""Blinded-adjudication join for gate-contribution-factorial.

Ported byte-for-byte (logic) from `placebo-seed-distribution-census/
apply_adjudication.py` (read in full before writing this) -- the join/CG1
mechanism is generic over {cell, arm, row_key, seed} keying and needed no
factorial-specific changes. Enforces the same UNBLINDING-ORDER GUARANTEE per
...
(graded file and id map matched by LINE, not by opaque_id lookup).
...
Two subcommands:
  ...
  apply --grading-manifest PATH
      ...
      writes. For every shard: verifies the committed hash (refuses
      otherwise), verifies the local shard pool file's sha256 still matches
      the committed pool manifest, unblinds the id map, computes PER-SHARD
      CG1 (gates_lib.cg1_evaluate_shard), then computes the POOLED
      clear-positive floor (gates_lib.cg1_pooled_clear_positive) across
      every shard that reached a PASS-or-attempt-1 result.
