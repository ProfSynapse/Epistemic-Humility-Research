#!/usr/bin/env python3
"""Dial token-logprob baseline v3 -- fresh self-consistent generation (GPU + CPU).

Registered in experiments/dial-logprob-baseline-v3/AMENDMENT.md. v1
(resolved 2026-07-18) and v2 (resolved 2026-08-13) both stopped at their
integrity gate trying to REPRODUCE a past cached generation exactly. v3 drops
that bet entirely: ONE fresh generation per row supplies the answer text, the
correctness label, the logprob variants, AND (via a teacher-forced pass over
the SAME captured token IDs) the dial's hidden-state input. There is no
external cached artifact to round-trip against.

CAPABILITY-CHECK FINDING (NOTEBOOK.md 2026-08-13, done before this module was
written): the pinned vllm==0.27.1 returns generated token IDs
(`CompletionOutput.token_ids`) and per-token logprobs
(`SamplingParams(logprobs=0)` -> `CompletionOutput.logprobs`) directly from a
normal generate() call -- certain, standard. It does NOT expose per-token
hidden states at an arbitrary intermediate layer from that same call: the
only hidden-state mechanism in this version (`extract_hidden_states`) is a
speculative-decoding/EAGLE-draft-model-training method requiring a
speculative_config + draft model config + KV-transfer connector + file-based
retrieval, architecturally unrelated to a simple per-request readout, and
nowhere near the batched-generation reference doc's required bridge
validation. The AMENDMENT's REGISTERED FALLBACK therefore applies: a
teacher-forced HF transformers forward pass over each row's captured token
IDs (prompt + generated, verbatim, never re-tokenized), at the arm's pinned
dial layer, executed in the SAME pinned stack/venv as the vLLM generation
(see cell.yaml `engine` block).

PER ARM (s_base_primary / t_deployed_descriptive), THREE PHASES:

  1. GENERATE (GPU, vLLM). Build the arm's prompt pool via `build_pool`
     (imported UNCHANGED from amendment_s_correctness_probe_extract.py -- S
     and T draw from the IDENTICAL pool; T's own extractor imports build_pool
     from S's module unchanged). Render each prompt with the arm's system
     prompt via the SHARED `render_probe_prompt` helper (thinking-off pinned
     and self-checked). ONE batched (chunked, resumable) vLLM generate() call
     per chunk captures token IDs and per-token logprobs together
     (`capture_row`). Score correctness fresh with the source cells' own
     scorer (`scorers.is_correct` / `is_stated_confidence_refusal`, imported
     unchanged). `select_attempted` then replays the S/T extractors' own
     sequential early-stop selection rule (stop once
  ...
  3. SCORE (CPU-only, no model). `score_arm_v3` implements LP3-G0 (a)
     capture integrity -- the teacher-forced pass's input ids are asserted,
     per row, to equal the phase-1 capture's own prompt+span ids; (b)
     coverage -- every attempted item has a recorded disposition; (c) power
     floor (>=1000 answered rows); (d) instrument sanity (fresh S-arm dial
     OOF AUROC >= 0.75, a sanity bound, NOT a reproduction target -- there is
     nothing to reproduce in v3). Only if all four hold does it compute the
     dial-vs-logprob paired-bootstrap margin (`paired_bootstrap_delta`,
     reused unchanged) and evaluate LP3-G1.
...
CLI (`main`):
  ...
  --arm ID --phase all (default)   all three phases in one process. NOTE:
                                    ...
                                    for guaranteed release, especially for the
                                    T arm where headroom is tighter (flagged
                                    in NOTEBOOK.md, not resolved by this
...
# Reused BY IMPORT, never retyped: same pool, same content-end trimming, same
# system prompts as the source extractors' own runs (AMENDMENT.md "Design").
...
def capture_row(item: dict, output, tokenizer, special_ids: set[int]) -> dict:
    ...
    the single source every downstream quantity derives from (LP3-G0a). The
...
def score_arm_v3(
    ...
    """LP3-G0 (a-d) then, only if all pass, LP3-G1. No reproduction target
    anywhere -- v3 has no external cached artifact to reproduce against."""
    by_key_capture = {r["row_key"]: r for r in captures}
    by_key_extract = {r["row_key"]: r for r in extraction_records}

    # LP3-G0(a): capture integrity -- the teacher-forced pass consumed
    ...
    # LP3-G0(b): coverage -- every attempted item has exactly one recorded
    ...
    if not abc_pass:
        result["lp3_g0"]["pass"] = False
        result["gate_verdict"] = {"stopped_at_lp3_g0": True}
        return result

    # Dial refit: fresh OOF AUROC at the pinned dial layer. No reproduction
    # target -- LP3-G0(d) is a sanity floor, applied to the S arm only
    # (AMENDMENT.md LP3-G0d: "the fresh S-arm dial OOF AUROC").
    ...
    instrument_sanity_ok = (
        dial_auroc >= instrument_sanity_min if arm.gate == "LP3-G1" else True
    )
    result["lp3_g0"]["d_instrument_sanity_ok"] = instrument_sanity_ok
    result["lp3_g0"]["d_dial_auroc"] = round(dial_auroc, 4)
    result["lp3_g0"]["d_instrument_sanity_auroc_min"] = instrument_sanity_min
    result["lp3_g0"]["pass"] = instrument_sanity_ok
    ...
    if arm.gate == "LP3-G1":
        floor = float(_gate(gates_cfg, "LP3-G1")["floor"])
...
def main(argv: list[str] | None = None) -> int:
    ...
    if a.phase in ("generate", "all"):
        ...
        if a.phase == "generate":
            print(f"[lp-v3] generation phase done -> {gen_runlog}", file=sys.stderr)
            return 0
        del engine
    ...
    if a.phase in ("extract", "all"):
        if not gen_runlog.exists():
            print(f"[lp-v3] extract phase requires generation runlog: {gen_runlog}", file=sys.stderr)
            return 2
        ...
        if a.phase == "extract":
            print(f"[lp-v3] extraction phase done -> {ext_runlog}", file=sys.stderr)
            return 0

    if a.phase in ("score", "all"):
        if not gen_runlog.exists() or not ext_runlog.exists():
            print("[lp-v3] score phase requires both generation and extraction runlogs",
                  file=sys.stderr)
            return 2
        captures = _read_jsonl(gen_runlog)
        attempted = select_attempted(captures, captures, pi["target_correct"],
        ...
        out_path = out_dir / f"lp_v3_{arm.id}_result.json"
