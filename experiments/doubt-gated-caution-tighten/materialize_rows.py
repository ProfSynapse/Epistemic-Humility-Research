...
COMMITTED ID-only `analysis-committed/split_manifest.json` (row_key, role,
split -- NO question text, NO aliases) against question text fetched at run
time from the private HF staging repo, mirroring the sibling two-signal
experiment's `materialize_eval_pool.py` / `analysis-committed/PROVENANCE.md`
containment scheme EXACTLY (read in full before writing this).
...
EXTRACT_MANIFEST_PATH = ANALYSIS / "l34_anchor_extract_manifest.json"
