...
| --- | --- | --- | --- | --- | --- |
| ap-veto-length-balanced-confirmatory | probe-fit | draft |  | On the raw base, does the post-generation correctness veto (post-L20 S/W/U dial) separate hallucinated answers from good answers by CONTENT rather than by answer length, once the classes are length-balanced and the 96-token truncation artifact is removed by extending the token budget? |  |
