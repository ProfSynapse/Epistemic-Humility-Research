#!/usr/bin/env python3
"""H4 ungated-vs-gated -- materialize the LOCAL, gitignored held-out row pool
...
a function of the prompt only (prompt_len-1, before any generation or write),
so the SAME tensors are valid input to this experiment's gate-decision math
without a fresh forward pass. This script only JOINS the promoted ID-only
split manifest against those existing artifacts and subsets them to
held-out rows; it fetches no text over the network.
...
Sources (read, never modified):
  ...
  - The resolved cell's own gitignored run artifacts, rows_with_text.jsonl
    (question + aliases + category_canon, already joined by that cell's own
    materialize_rows.py) and l34_anchor_extract.safetensors (L34 anchor
    vectors, prompt_len-1, keyed by sanitized row_key). Default source is the
    build worktree where that cell was actually run
...
def main(argv: list[str] | None = None) -> int:
    ...
    manifest = {
        "stage": "h4_ungated_vs_gated_heldout_materialize",
