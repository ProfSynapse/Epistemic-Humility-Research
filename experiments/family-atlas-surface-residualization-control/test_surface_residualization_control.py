...
SPEC = importlib.util.spec_from_file_location(
    "surface_control", HERE / "reanalyze_surface_residualization.py"
)
...
def test_surface_data_exhaust_is_private_aligned_and_content_addressed(
    ...
    rows = [
        {
            ...
            "category_canon": "unit",
            "split": "fit",
            "question": f"Question topicword{i} categoryword{i % 3}?",
            "rendered_prompt_token_count": 5 + i,
            "render_template_id": "unit",
        }
        for i in range(8)
    ]
    data = control.SourceData(
        name="fixture",
        spec={},
        root=tmp_path,
        ...
        splits=np.asarray(["fit"] * len(rows)),
        sources=np.asarray(["kuq"] * len(rows)),
        capture_files=[],
        estimator=None,
        committed_profile=[],
        private_texts={row["question"] for row in rows},
        provenance={},
    )
    features = control.build_surface_features(rows, config())
    report = control.write_surface_data_exhaust(data, np.arange(len(rows)), features)
    checkpoint = tmp_path / "checkpoints" / "fixture" / "combined" / "hs0.npz"
    checkpoint.parent.mkdir(parents=True)
    with checkpoint.open("wb") as fh:
        np.savez_compressed(fh, yhat=np.ones((len(rows), 3)))
    report = control.finalize_oof_data_exhaust(report, "fixture")
    assert report["fit_row_count"] == len(rows)
    assert report["fit_manifest_sha256"]
    assert report["surface_matrix_sha256"]
    assert report["oof_prediction_content_sha256"]
    assert report["oof_prediction_file_count"] == 1
    manifest = (tmp_path / "exhaust" / "fixture" / "fit_rows_private.jsonl").read_text()
