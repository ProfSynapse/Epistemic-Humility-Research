...
**Addendum, root cause of the dropped turn and a new launch gotcha (executor report, lead-verified where checkable).** The first relaunch attempt's dry-run background task was killed at turn end with a zero-byte output file, so no dry-run values were ever confirmed and no launch followed; the executor has adopted "never end a turn on an unconfirmed async wait". On the second attempt the executor self-caught a NEW failure mode before it cost anything: `docker run` without `--entrypoint python3` invokes the image's baked-in default entrypoint, a Studio/Jupyter/sshd bootstrap (npm build, supervisord) that runs indefinitely and never touches the trainer. A container in that state shows "Up" while doing nothing GPU-relevant, which defeats liveness checks that only test container existence. Detection: the log showed Jupyter/supervisord startup instead of trainer output; GPU stayed at 0 MiB. The stray container was stopped and force-removed, GPU confirmed clear, and the corrected dry-run passed (seed-3 DPO merged base, r32/alpha64/d0.05, reward loads, 14888 examples). The real launch's `Config.Entrypoint=["python3"]` verified via inspect. Gotcha queued for the local-runtime skill at the next doc pass: ALWAYS pass `--entrypoint python3` explicitly; a missing entrypoint produces a live-looking dead container. Dry-run row-text stdout was kept in an untracked scratchpad log only, per containment.

## 2026-08-06 19:22Z — `clean_sft_dpo_grpo` seed 3 training COMPLETE (relaunch), clean

Container `eh-grpo3seed-3-clean_sft_dpo_grpo-train-20260806T131003Z` exited **0** at 19:22:53Z, 6h13m, read from the wait output file. The backup poller reported `gone-from-daemon` because this launch used `--rm` (container self-removed on exit); per the standing rule that reading is terminal but not evidence either way, and the authoritative exit 0 plus artifacts settle it. The transient CUDA fault did not recur.
...
## 2026-08-07 ~07:45Z — RED-TEAM PASS COMPLETE: safe to resolve; findings adjudicated, G5 delivered, G4 closed

Adversarial review ran over the full block (goalpost integrity from git history, oracle leak, instrument findings, adjudication arithmetic, provenance, statistical honesty). Reviewer verdict: SAFE TO RESOLVE, no blocker, both gate verdicts survive independent re-derivation. Reviewer confirmations of record: gates.yaml and cell.yaml each have exactly one commit, sha byte-identical to the pin; G1/G2 bands committed 13h01m and 2h02m before their deciding artifacts existed; all 16 full-eval configs hash-identical on every non-arm section (sha 81ea92bf6164) with deterministic decode; G1 pairs differ ONLY in adapter null vs GRPO. Lead adjudication of the findings follows; every material claim was re-derived by the lead before acceptance.

**FINDING 1 (MAJOR, ACCEPTED): SelfAware train/eval contamination — standing limitation on all known-row metrics.**
117 distinct SelfAware eval questions (118 of 3369 rows, ALL answerable/known, ZERO unknown) appear verbatim as user-side training prompts in every dataset this block consumes. Lead re-derived independently: 117 exact-match distinct questions against the DPO train prompts, all label=known. The pre-registered leakage guard (`build_datasets.py:210`, PROTOCOL §377-381) covers ONLY the Cheng TriviaQA test set; SelfAware was never in scope, and G0's dataset_audit checks counts, not disjointness. Contamination is behaviorally active (contaminated-stratum over-refusal ~30% vs ~71% clean).
Gate impact, verified not assumed:
- G1 is STRUCTURALLY IMMUNE: both G1 metrics are computed over the 1032 unknown-labeled rows only (scorers.py:281-284) and zero unknown questions leak.
- G2 SURVIVES stratification. Lead re-derived seed 3: contaminated stratum -1.70 pp (+2 rows), clean stratum -1.85 pp (+41 rows); reviewer's seed-2 stratification shows the same uniformity (-0.85/-0.77). The recovery is uniform across strata; contaminated rows contribute ~their 5% population share.
Consequences: (a) absolute LEVELS of over_refusal_pct, correct_on_known_pct, truthful_pct are inflated by a memorized 5% of known rows and MUST carry this caveat into paper 2; within-lineage DELTAS on these metrics are stratum-robust. (b) FOLLOW-UP ITEM (registered here): extend the leakage guard to assert normalized-question disjointness against SelfAware before any future block trains on these datasets.

**FINDING 2 (MAJOR, ACCEPTED): G5 delivered — stage-ordering deltas, with a sign reversal the amendment's Effect 3 prose did not survive.**
G5 (g5_stage_ordering_comparison, non-gating) was registered but never computed until this pass. Over-refusal deltas (GRPO-first minus GRPO-last), lead re-derived from the verified G3 per-seed table:
- DPO pair (grpo_dpo - dpo_grpo): seed 1 -1.67, seed 2 +0.17, seed 3 +2.18. Mean +0.23 [min -1.67, max +2.18]. **SIGN REVERSES at both new seeds.** The amendment pre-declared this pair's seed-1 margin unadjudicable and barred it from being reported as a finding; that bar did its job. Effect 3's prose ("both matched pairs point the same way") is FALSE at three seeds for the DPO pair and must not be recited in any write-up.
- KTO pair (grpo_kto - kto_grpo): seed 1 -5.78, seed 2 -3.13, seed 3 -2.49. Mean -3.80 [min -5.78, max -2.49]. Direction holds at all three seeds, attenuating from seed 1. Descriptive only, per the gate.
Intervals use the same seed-level construction as G3 (n=3, descriptive spread, not inferential).

**FINDING 3 (MINOR, ACCEPTED): G4 evaluated and NOT TRIGGERED.**
Trigger requires >200 distinct stated_confidence values AND Brier vs response-appropriateness <0.35 on any seed-2/3 arm. Distinct values range 4-85 across arms (lead re-derived the max: clean_schema_sft_merged_seed3 = 85 exactly). The Brier leg alone is met on the two clean-SFT base arms (0.3460/0.3436) but the distinct-value leg is nowhere near threshold. Confidence stays collapsed exactly as pre-stated; nothing routes to a new registration.

**FINDING 4 (MINOR, ACCEPTED into G2 limitations):** G2's comparison carries one uncontrolled LoRA-merge round-trip (denominator = base+GRPO adapter attached; numerator = 16-bit merge of that combination + DPO adapter), and its numerator is a DPO arm whose LoRA init is frozen at trainer baseline 3407 across seeds (the condition the 2026-07-31 seed-mirroring ruling called weakening; source model and data order still vary, so seeds 2/3 are partial, not absent, replicates). Together with G2 being direction-only by design, the G2 PASS is real but thin: an 18-row and 43-row effect, to be reported with counts, never percentages alone.
