"""SFT clean response-confidence seed-3 full local config.

train_sft.py's `--config` flag imports this file as a Python module via
importlib and calls `Config()` (see train_sft.py:590-597); a YAML config does
not load through this path (confirmed at seed 2 — see
sft_schema_clean_response_confidence_seed2_full_config.py's docstring for the
full archaeology of that finding).

Cloned directly from the seed-2 working config
(experiments/grpo-three-seed-confirmatory/configs/sft_schema_clean_response_confidence_seed2_full_config.py,
which produced sft_schema_clean_seed2_full/20260731_232307) with exactly three
fields changed for seed 3: `config.seed`, `config.lora.random_state`, and
`config.training.output_dir`. Every other value is carried unchanged,
matching cell.yaml's frozen `clean_sft` arm hyperparameters exactly (stage 1,
source foundation, trainer synaptic-tuner/Trainers/sft/train_sft.py, dataset
sft_clean, num_epochs 1).

lora.random_state: 3 per the standing lead ruling (NOTEBOOK.md "LEAD RULING:
lora.random_state mirrors the seed number") — mirrors seed: 3.
"""
...
def Config():  # noqa: N802 - train_sft.py expects this exact symbol.
    ...
    config.training.output_dir = "scratch/schema_response_confidence/runs/sft_schema_clean_seed3_full"
