"""Generation-side grading helpers for rr2-mistral-adjudicated-refusal-confirm.

Ported (logic) from `experiments/rr-cross-family-raw-refusal/gen_lib.py`: same
generation contract (min_new_tokens=1, EOS enabled, greedy, MAX_NEW_CAP=200)
and the same `clean_tighten` readout (reported for continuity with RR).
`grade_row` is EXTENDED beyond RR's version: it merges RR's locked v1 grade
(`grader.grade_one`, this experiment's `refused_v1`/`well_formed_correct`
comparability fields) with the new v2 grade (`detector_v2.grade_one_v2`,
...
re-deriving anything. Per AMENDMENT.md's data-exhaust rule, this full
sub-grade dict (not a collapsed boolean) is what gets persisted to the
gitignored row-level run log.
"""
...
def grade_clean_tighten(raw_text: str, terminated_naturally: bool) -> dict:
    """clean_tighten metric, ported verbatim from RR's gen_lib.py: the first
    parsed JSON answer is a v1-canonical refusal AND generation terminated
    naturally AND single answer field, no post-JSON repetition. Reported for
    continuity with RR; never this experiment's gate (the adjudicated
    `refused_final` is)."""
    text = raw_text or ""
...
def grade_row(text: str, terminated_naturally: bool, aliases: list[str] | None) -> dict:
    ...
    (grader.py, reported for RR comparability) + v2 refused_v2/matched_pattern_ids
