"""Grader for rr2-mistral-adjudicated-refusal-confirm ("v1"/RR-locked detector).

Byte-for-byte identical (except this docstring) to
`experiments/rr-cross-family-raw-refusal/grader.py`, ported verbatim per this
repo's convention of each experiment directory owning its own copy. Reused
here so `refused_v1`/`clean_tighten` stay directly comparable to RR's own
numbers (AMENDMENT.md "detector-v2-only rates are reported alongside every
gated rate for comparability with RR"). `detector_v2.py` imports this module's
`is_degenerate`, `_is_stated_confidence_refusal`, `_normalize_aliases`, and
`_is_correct` helpers directly so the v2 detector differs from v1 ONLY in
which refusal-phrase surface it matches, not in degenerate/correctness logic.
