...
Ported (logic) from `experiments/rr-cross-family-raw-refusal/steer_lib.py`:
same direct InterventionHook/GenerationInterventionController/RunLog driving
(not the declarative `mechinterp steer` YAML-recipe path), same batched
fixed-generation contract. `load_model` now sets THIS experiment's namespaced
render env vars (RR2_RENDER_MODEL/RR2_RENDER_REVISION, matching this
experiment's own `render.py`) rather than RR's RR_RENDER_MODEL/RR_RENDER_REVISION
-- carrying forward RR's own late-discovered fix (NOTEBOOK.md
"RENDER-ENV-VAR FIX": `load_model` must set these BEFORE loading, at the one
call site every caller already shares, so the two can never drift apart or
be forgotten) rather than re-introducing the gap it closed.

Single substrate only (Mistral-7B-Instruct-v0.3, plain AutoModelForCausalLM,
standard attention), so -- like RR -- this module does not impose a
CUDA_LAUNCH_BLOCKING requirement.
"""
...
def load_model(model_name: str, revision: str):
    ...
    render-side env vars (RR2_RENDER_MODEL/RR2_RENDER_REVISION) to the SAME
    model/revision, in the same place, so the two can never drift apart or
    be forgotten by a caller -- carrying forward RR's own fix for the exact
    same gap (NOTEBOOK.md RENDER-ENV-VAR FIX)."""
    import torch
    from transformers import AutoModelForCausalLM, AutoTokenizer

    os.environ["RR2_RENDER_MODEL"] = model_name
    os.environ["RR2_RENDER_REVISION"] = revision or ""
...
def run_batch_fixed(
    ...
    RR's own harness used and the sibling H3 amendment's termination-rule
    regression was about; this is NOT reintroduced here."""
    import torch
...
def run_rows(
    ...
    plus a separate baseline lookup for inactive rows, mirroring RR's own
    ...
    screen without any re-generation."""
