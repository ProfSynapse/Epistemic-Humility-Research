...
margin-separation-fine-ladder (M1b).

Direct InterventionHook/GenerationInterventionController/RunLog driving,
ported (logic) BYTE-IDENTICAL from `margin-mapping/steer_lib.py` (read in
full before writing this): same `erase_write`/`anchor_onward`
...
differ (M1B_RENDER_MODEL/M1B_RENDER_REVISION instead of
MARGIN_RENDER_MODEL/MARGIN_RENDER_REVISION), carrying forward the same
RR/RR2/RR3/midband-heldout/factorial/M1 render-env-namespacing convention.

Single dose per pass: every row in one `run_rows` call shares the same rung
(cell.yaml `ladder`: every row dosed at every ladder step for the population
in question -- M1b has no gate, so this is simply "one call per rung").
"""
...
def load_model(model_name: str, revision: str):
    """Loads the generation model/tokenizer AND sets M1B_RENDER_MODEL/
    M1B_RENDER_REVISION to the SAME model/revision, in the same place, so
    the two can never drift apart. Qwen3.5 requires transformers >= 5.x (run
    under /home/profsynapse/miniconda3/bin/python3, base conda, per M1's own
    documented deviation-with-cause)."""
    import torch
    from transformers import AutoModelForCausalLM, AutoTokenizer

    os.environ["M1B_RENDER_MODEL"] = model_name
    os.environ["M1B_RENDER_REVISION"] = revision or ""
...
def run_batch_fixed(
    ...
    eos-anywhere, the RR/RR2/RR3/midband-heldout/factorial/M1 convention."""
    import torch
