"""JSON-answer render for margin-separation-fine-ladder (M1b).

Ported (logic) byte-identical to `margin-mapping/render.py` (read in full
before writing this). Prompt contract, anchor position, and thinking-off
self-check are unchanged; only the environment variable names are
namespaced to THIS experiment (M1B_RENDER_MODEL / M1B_RENDER_REVISION,
following M1's own namespacing convention: M1 renamed the factorial's
GATEFACT_RENDER_* to MARGIN_RENDER_*) so this render cache can never collide
with any sibling experiment's own render module in the same process.
"""
...
def assert_no_think_scaffolding(rendered_prompt: str, *, model: str) -> None:
    ...
    (enable_thinking=False pin, reused from M1/the factorial's generation
    stack)."""
...
def _log_fallback_once(model: str, mode: str, detail: str) -> None:
    ...
    print(f"[m1b-render] {model}: {detail}", file=sys.stderr, flush=True)


def _tokenizer():
    global _TOKENIZER, _TOKENIZER_KEY
    model = os.environ.get("M1B_RENDER_MODEL")
    if not model:
        raise RuntimeError("M1B_RENDER_MODEL must name the HF tokenizer repo")
    revision = os.environ.get("M1B_RENDER_REVISION") or None
    key = (model, revision)
...
def render(row: dict) -> str:
    ...
    tok = _tokenizer()
    model = os.environ.get("M1B_RENDER_MODEL", "<unset>")
