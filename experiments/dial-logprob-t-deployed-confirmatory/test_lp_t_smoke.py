#!/usr/bin/env python3
"""Fixture-backed smoke for lp_t_harness.py -- drives the REAL entry points
(`run_arm_generation`, `run_arm_extraction`, `score_arm_t`, `select_attempted`,
...
`containment`). Adapted verbatim from
experiments/dial-logprob-baseline-v3/test_lp_v3_smoke.py for this cell's
single gated arm (LT-G0/LT-G1 in place of v3's LP3-G0/LP3-G1); no v3 file is
modified.
...
internally consistent (the LT-G0a capture-integrity check is exercised for
real, not stubbed out).

Run: python3 -m pytest experiments/dial-logprob-t-deployed-confirmatory/test_lp_t_smoke.py -v
"""
...
import lp_t_harness as lp  # noqa: E402
...
def _fixture_arm(system_prompt: str) -> lp.ArmConfig:
    return lp.ArmConfig(
        id="t_deployed_confirmatory", model_name="fixture-model", adapter=None,
        quantization=None, lora=False, dial_layer=DIAL_LAYER, gate="LT-G1",
...
def test_run_arm_extraction_consumes_exact_captured_ids_and_score_arm_full_pass(tmp_path):
    ...
    result = lp.score_arm_t(arm, captures, resumed, tensors_dir, n_boot=200, seed=20260813,
                              gates_cfg=gates_cfg, power_floor_n=5, instrument_sanity_min=0.0)
    assert result["lt_g0"]["a_capture_integrity_ok"] is True, result["lt_g0"]
    assert result["lt_g0"]["a_n_integrity_fail"] == 0
    assert result["lt_g0"]["b_coverage_ok"] is True
    assert result["lt_g0"]["c_power_floor_ok"] is True
    assert result["lt_g0"]["d_instrument_sanity_ok"] is True
    assert result["lt_g0"]["pass"] is True
    ...
    assert set(result["gate_verdict"]) == {"LT_G1_pass", "falsifier_fired", "ambiguous_band"}


def test_score_arm_t_stops_at_lt_g0a_when_capture_integrity_fails(tmp_path):
    """LP3-G0(a) must have teeth: corrupt one extraction record's
    teacher_forced_input_ids and confirm score_arm_t catches it and stops,
    ...
    gates_cfg = lp.load_gates_config(lp.GATES_YAML)
    result = lp.score_arm_t(arm, captures, corrupted, tensors_dir, n_boot=200, seed=20260813,
                              gates_cfg=gates_cfg, power_floor_n=5, instrument_sanity_min=0.0)
    assert result["lt_g0"]["a_capture_integrity_ok"] is False
    assert result["lt_g0"]["a_n_integrity_fail"] == 1
    assert result["lt_g0"]["pass"] is False
    assert result["gate_verdict"] == {"stopped_at_lt_g0": True}


def test_score_arm_t_stops_at_lt_g0c_power_floor(tmp_path):
    model, tokenizer, device, engine, arm, pool_items = _make_fixture()
    params = _StubSamplingParams(max_tokens=arm.max_new_tokens)
    gen_runlog = tmp_path / "gen_runlog.jsonl"
    captures = lp.run_arm_generation(engine, tokenizer, arm, pool_items, gen_runlog, params)
    ...
    gates_cfg = lp.load_gates_config(lp.GATES_YAML)
    result = lp.score_arm_t(arm, captures, ext_records, tensors_dir, n_boot=200, seed=20260813,
                              gates_cfg=gates_cfg, power_floor_n=1000, instrument_sanity_min=0.0)
    assert result["lt_g0"]["c_power_floor_ok"] is False
    assert result["lt_g0"]["pass"] is False
    assert result["gate_verdict"] == {"stopped_at_lt_g0": True}


def test_score_arm_t_stops_at_lt_g0d_instrument_sanity(tmp_path):
    model, tokenizer, device, engine, arm, pool_items = _make_fixture()
    params = _StubSamplingParams(max_tokens=arm.max_new_tokens)
    gen_runlog = tmp_path / "gen_runlog.jsonl"
    captures = lp.run_arm_generation(engine, tokenizer, arm, pool_items, gen_runlog, params)
    ...
    assert result["lt_g0"]["a_capture_integrity_ok"] is True
    assert result["lt_g0"]["c_power_floor_ok"] is True
    assert result["lt_g0"]["d_instrument_sanity_ok"] is False
    assert result["lt_g0"]["pass"] is False
    assert result["gate_verdict"] == {"stopped_at_lt_g0": True}
...
def test_dry_run_detects_present_and_missing_inputs():
    ...
    # This cell's single arm's checkpoint/adapter dirs are local paths
    # (verbatim from v3's t_deployed_descriptive arm); the only reason this
    # could return 2 is a missing local checkpoint/adapter or datasets/ file,
    # which is a real repo-state fact this dry-run is supposed to surface,
    # not mask.
    assert code_ok in (0, 2)

    cell_missing = json.loads(json.dumps(cell))
    cell_missing["arms"][0]["model"]["adapter"] = "scratch/does-not-exist/adapter"
