...
def load_model(model_name: str, revision: str):
    """Loads the generation model/tokenizer AND sets MARGIN_RENDER_MODEL/
    MARGIN_RENDER_REVISION to the SAME model/revision, in the same place, so
    the two can never drift apart. Qwen3.5 requires transformers >= 5.x (run
    under /home/profsynapse/miniconda3/bin/python3, base conda, per
    midband-heldout's own documented deviation-with-cause)."""
    import torch
    from transformers import AutoModelForCausalLM, AutoTokenizer

    os.environ["MARGIN_RENDER_MODEL"] = model_name
    os.environ["MARGIN_RENDER_REVISION"] = revision or ""
