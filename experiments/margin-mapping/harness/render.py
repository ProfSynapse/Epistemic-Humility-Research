"""JSON-answer render for margin-mapping (M1).

Ported (logic) byte-identical to `gate-contribution-factorial/render.py`
(read in full before writing this, which itself ports RR3/census's own
...
experiment (MARGIN_RENDER_MODEL / MARGIN_RENDER_REVISION) so this render
cache can never collide with any sibling experiment's own render module in
the same process. Family-agnostic: `steer_lib.load_model` sets these env
vars to whichever family/revision is currently loaded.
"""
...
def assert_no_think_scaffolding(rendered_prompt: str, *, model: str) -> None:
    ...
    (enable_thinking=False pin, reused from the factorial's generation
    stack)."""
...
def _log_fallback_once(model: str, mode: str, detail: str) -> None:
    ...
    print(f"[margin-render] {model}: {detail}", file=sys.stderr, flush=True)


def _tokenizer():
    global _TOKENIZER, _TOKENIZER_KEY
    model = os.environ.get("MARGIN_RENDER_MODEL")
    if not model:
        raise RuntimeError("MARGIN_RENDER_MODEL must name the HF tokenizer repo")
    revision = os.environ.get("MARGIN_RENDER_REVISION") or None
    key = (model, revision)
...
def render(row: dict) -> str:
    ...
    tok = _tokenizer()
    model = os.environ.get("MARGIN_RENDER_MODEL", "<unset>")
