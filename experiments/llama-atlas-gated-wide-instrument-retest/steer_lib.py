"""Batched generation + activation-write driver, shared by dose_ladder.py and
heldout_scorer.py.

Direct InterventionHook/GenerationInterventionController/RunLog driving
(execution-model adjudication, NOTEBOOK.md): this mirrors
`qwen35-4b-midband-doubt-snap/run_dose_ladder.py`'s `run_batch_fixed` /
`run_baseline` / `run_arm_dose` (read in full before writing this) rather
than the declarative `mechinterp steer` YAML-recipe path
`doubt-snap-cross-family-confirmatory/prep_tuner_cell.py:materialize_dose_sweep`
uses. AMENDMENT.md's Execution section names both paths as available for the
local 3090 lane; this harness follows the more recently established
direct-Python pattern, which is also what this repo's H3/H4/H6 harnesses
used and CPU-smoke-tested successfully.

Both RR substrates (Llama-3.2-3B-Instruct, Mistral-7B-Instruct-v0.3) are
plain `AutoModelForCausalLM` loads with standard (non-hybrid) attention, so
unlike the Qwen3.5 ladder this module does NOT impose a CUDA_LAUNCH_BLOCKING
requirement -- that was specific to Qwen3.5's custom chunk_gated_delta_rule
op under an active hook across many decode steps (run_dose_ladder.py's own
`check_env` docstring), which does not apply to either RR family. Flagged in
NOTEBOOK.md as an explicit choice, not an oversight.
"""
...
def run_batch_fixed(
    ...
    `qwen35-4b-midband-doubt-snap/run_dose_ladder.py:run_batch_fixed`."""
    import torch
...
def run_rows(
    ...
    """Generic batched runner: `gains` maps row_key -> per-row gain (0.0 for
    an inactive/no-write row within an active pass, e.g. a known row not
    selected by the doubt gate but sharing this arm's batched call is NOT
    how this harness handles mixed active/inactive rows -- see
    dose_ladder.py/heldout_scorer.py callers, which instead partition rows
    into an active-only batch call (gain != 0 for every row in the call)
    plus a separate reused-baseline lookup for inactive rows, exactly
    mirroring run_dose_ladder.py's own `known_all_covered` pattern. This
    function assumes every row passed in IS active for this call; it does
    not itself branch on gain == 0."""
    ...
    for i in range(0, len(pending), batch_size):
        ...
        for row, res in zip(batch, gen):
            ...
            run_log.record(log_key_fn(row), {
                ...
                "category_canon": row.get("category_canon"), "gain": gains[row["row_key"]],
