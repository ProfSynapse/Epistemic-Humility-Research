"""Direction/gate fit module (G0) for rr-cross-family-raw-refusal.

Fits u_d (doubt sensor), c_hat (snap write direction), and the
random_direction placebo at one (family, candidate_layer) atlas site, fresh
on FIT rows only. This is a byte-for-byte port of the recipe in
`doubt-snap-cross-family-confirmatory/prep_tuner_cell.py:fit_directions` /
`fit_byte_identical`, generalized to take an arbitrary layer index and an
in-memory anchor dict instead of one tuner batch-capture call per layer --
the same generalization `qwen35-4b-midband-doubt-snap/fit_midband_directions.py`
already made for its own mid-band layers (read in full before writing this;
`_fit_all` there is the same formula with a different loop shape).
