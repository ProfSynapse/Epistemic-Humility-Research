"""Fixed generation + honest clean_tighten metric for the Stage C dose ladder.

Byte-for-byte port (logic, not import -- sibling branch not checked out in
this worktree) from `doubt-snap-cross-family-confirmatory/gen_lib.py` (main
checkout `/home/profsynapse/code/Epistemic-Humility-Research`, read in full
before copying). Diffed against `j-space-midband-write-sweep-qwen3-4b/gen_lib.py`
(available in this worktree): functionally identical (same MAX_NEW_CAP,
resolve_eos_ids, grade_clean_tighten, _find_first_json_object), docstrings
differ only. `run_pass_fixed` below is kept for citation/single-row parity;
`run_dose_ladder.py` uses its own batched variant (`run_batch_fixed`) since
Stage C batches multiple rows per `model.generate()` call, which this ported
function does not support.

This instrument locks the fixed generation contract (AMENDMENT.md
"registered readouts"): min_new_tokens=1, eos_token_id includes <|im_end|>,
enable_thinking=False (baked into the render function), greedy
(do_sample=False), max_new_tokens capped at MAX_NEW_CAP=200.

Only the anchor_onward write scope is implemented (this amendment's registered
design, mirroring the late-site/mid-band precedents).
"""
...
def run_pass_fixed(
    ...
    source. NOT used by run_dose_ladder.py's batched loop (see module
    docstring); kept so a diff against the cited source stays legible."""
