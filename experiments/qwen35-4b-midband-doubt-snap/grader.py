"""Grader for the qwen35-4b-midband-doubt-snap Stage C dose ladder.

Byte-for-byte port (logic, not import -- the source lives on a sibling branch
not checked out in this worktree) from
`doubt-snap-cross-family-confirmatory/grader.py` (main checkout
`/home/profsynapse/code/Epistemic-Humility-Research`, read in full before
copying). `is_degenerate` is the coherence-floor check (empty / no-alnum /
repeated word n-gram / repeated character n-gram, including the
JSON-answer-field-aware and no-whitespace-loop fixes that source module's own
docstring documents finding necessary on this exact surface's real
generations), and `grade_one` adds `well_formed_correct` for the
known-correct cost-control population. Diffed against
`j-space-midband-write-sweep-qwen3-4b/grader.py` (available in this worktree)
before copying: that sibling module depends on an external `scorers` module
(`experiment/phase1/eval/scorers.py`) for alias-matching/refusal-detection,
whereas this copy is the CROSS-FAMILY grader's self-contained version
(`_normalize_answer_text`/`_is_correct`/`_is_stated_confidence_refusal`
defined locally) -- the two are not interchangeable, and the amendment's
harness-builder task explicitly named the cross-family grader as the mirror
source, so this file is that one, not the j-space one.
