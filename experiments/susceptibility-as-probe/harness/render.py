"""JSON-answer render for susceptibility-as-probe (M2) -- the readout
capture channel's prompt.

Ported (logic) byte-identical to `margin-mapping/harness/render.py` (read in
full before writing this, which itself ports `gate-contribution-factorial/
render.py`). Prompt contract, anchor position, and thinking-off self-check
are unchanged; only the environment variable names are namespaced to THIS
experiment (M2_CAPTURE_RENDER_MODEL / M2_CAPTURE_RENDER_REVISION) so this
render cache can never collide with any sibling experiment's own render
module in the same process, and so the readout capture prompt is exactly the
"baseline render stack (atlas conventions)" cell.yaml's `channels.readout.
capture.render` names -- the SAME system prompt + user question, NO
confidence-elicitation suffix (that lives in render_confidence.py).
"""
...
def assert_no_think_scaffolding(rendered_prompt: str, *, model: str) -> None:
    ...
    (enable_thinking=False pin, reused from the margin-mapping/factorial
    generation stack)."""
...
def _log_fallback_once(model: str, mode: str, detail: str) -> None:
    ...
    print(f"[m2-capture-render] {model}: {detail}", file=sys.stderr, flush=True)


def _tokenizer():
    global _TOKENIZER, _TOKENIZER_KEY
    model = os.environ.get("M2_CAPTURE_RENDER_MODEL")
    if not model:
        raise RuntimeError("M2_CAPTURE_RENDER_MODEL must name the HF tokenizer repo")
    revision = os.environ.get("M2_CAPTURE_RENDER_REVISION") or None
    key = (model, revision)
...
def render(row: dict) -> str:
    ...
    tok = _tokenizer()
    model = os.environ.get("M2_CAPTURE_RENDER_MODEL", "<unset>")
