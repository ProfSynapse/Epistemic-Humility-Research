...
Reads the row-level (gitignored) `analysis/adjudication_applied.jsonl` --
written by `apply_adjudication.py cmd_apply` once every grading shard has
passed CG1 -- joins baseline vs each per-seed dosed arm over the fixed S
subsample rows via `paired_delta.paired_delta_pts`, evaluates the SURVIVES/
RETIRED/INDETERMINATE criterion per family
(`criterion.evaluate_family_with_committed_sign` /
...
def build_family_report(family: str, applied_rows: list[dict[str, Any]], s_row_keys: list[str]) -> dict[str, Any]:
    baseline_by_key = index_by_key([r for r in applied_rows if r["cell"] == family])
    dosed_by_seed = index_by_seed_and_key([r for r in applied_rows if r["cell"] == family])

    per_seed: list[dict[str, Any]] = []
    deltas_pts: list[float] = []
    seeds_present = sorted(dosed_by_seed.keys())
    for seed in seeds_present:
        pd = paired_delta_pts(dosed_by_seed[seed], baseline_by_key, s_row_keys)
        per_seed.append({"seed": seed, **pd})
...
def build_report(applied_rows: list[dict[str, Any]], subsample_manifest: dict[str, Any]) -> dict[str, Any]:
    families_report = {}
    for family in config.FAMILIES:
        s_row_keys = subsample_manifest["families"][family]["row_keys"]
        families_report[family] = build_family_report(family, applied_rows, s_row_keys)
    return {
...
def cmd_build(args: argparse.Namespace) -> int:
    ...
    report = build_report(applied_rows, subsample_manifest)
