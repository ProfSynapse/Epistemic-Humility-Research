#!/usr/bin/env python3
"""Blinded, SHARDED grading-pool builder for placebo-seed-distribution-census
(cell.yaml `wide_instrument.adjudication`; gates.yaml `sc2_grading_integrity`).

Adapted from `rr3-corrected-placebo-replication/build_adjudication_pool.py`
(read in full before writing this), simplified for this census's flatter arm
shape: THREE cells (one per family: qwen35_4b, mistral7b_v03, llama32_3b),
each carrying that family's shared baseline S rows PLUS K=15 dosed seed
passes over the same S rows -- no gated arm, no dose ladder (this experiment
has neither; AMENDMENT.md "Design").

Dedup key is (cell, arm, row_key, seed) -- arm in {"baseline",
"random_direction"}; a row_key legitimately carries K distinct dosed texts
(one per seed) plus one baseline text.

DECOYS:
  clear_negative  drawn from a HELD-BACK pool (successor fix (a)): an undosed
                  baseline pass over rows disjoint from every scored S-row
                  population by construction. NOT YET GENERATED by this
                  harness build (would require an additional GPU pass beyond
                  the registered K=15xS smokes this build is authorized to
                  run); `load_heldback_candidates` raises loudly if the
                  expected runlog is absent rather than silently degrading
                  the decoy source. Pool-assembly LOGIC is fully exercised on
                  synthetic fixtures by test_census_smoke.py regardless.
  clear_positive  drawn from core's own refused_v2==True rows in the
                  random_direction arm -- disjoint from core by construction.
...
CLEAR_POSITIVE_DECOYS_PER_SHARD_FLOOR = 25  # gates.yaml sc2_grading_integrity.clear_positive_decoys_per_shard_floor

CELLS = ("qwen35_4b", "mistral7b_v03", "llama32_3b")
...
def load_family_rows(family: str, seeds: list[int]) -> list[dict[str, Any]]:
    ...
    for seed in seeds:
        out += _normalize(common.load_jsonl(runlog_path(f"{family}__random_direction__seed{seed}")), cell=family, arm="random_direction", seed=seed)
    return out
...
def load_heldback_candidates() -> list[dict[str, Any]]:
    """Held-back clear-negative decoy candidates (successor fix (a)): an
    undosed baseline pass over rows disjoint from every scored S-row
    population, per family. NOT generated by this harness build (see module
    docstring); raises loudly rather than silently falling back to an
    unsafe decoy source."""
    out: list[dict[str, Any]] = []
    missing = []
    for family in CELLS:
    ...
    if missing:
        raise SystemExit(
            ...
            f"This harness build did not generate the held-back clear-negative "
            f"decoy source pass (out of scope for the registered GPU smokes); "
            f"run the (not-yet-written) held-back generation pass before "
            f"building a REAL grading pool. Pool-assembly logic itself is "
            f"fully tested against synthetic fixtures in test_census_smoke.py."
        )
    return out
