"""JSON-answer render for placebo-seed-distribution-census.

Ported (logic, not cross-experiment import, per this repo's convention of
each experiment directory owning its own copy) from
`experiments/rr3-corrected-placebo-replication/render.py`, read in full
...
to THIS experiment (CENSUS_RENDER_MODEL / CENSUS_RENDER_REVISION) so this
render cache can never collide with RR3's own render module
(RR3_RENDER_MODEL/RR3_RENDER_REVISION), RR2's, or RR's if ever imported in
the same process. Family-agnostic: `steer_lib.load_model` sets these env vars
to whichever family/revision is currently loaded, so the SAME render.py
serves all three census families, never simultaneously in one process.
"""
...
def _log_fallback_once(model: str, mode: str, detail: str) -> None:
    ...
    print(f"[census-render] {model}: {detail}", file=sys.stderr, flush=True)


def _tokenizer():
    global _TOKENIZER, _TOKENIZER_KEY
    model = os.environ.get("CENSUS_RENDER_MODEL")
    if not model:
        raise RuntimeError("CENSUS_RENDER_MODEL must name the HF tokenizer repo")
    revision = os.environ.get("CENSUS_RENDER_REVISION") or None
    key = (model, revision)
...
def render(row: dict) -> str:
    ...
    tok = _tokenizer()
    model = os.environ.get("CENSUS_RENDER_MODEL", "<unset>")
