"""Grader for the cross-family J-space layer contrast.

Ported unchanged from `j-space-midband-write-sweep-qwen3-4b/grader.py` (which
itself ports the resolved doubt-gated-caution-tighten predecessor). This
module is model-agnostic (it only inspects generated text against gold
aliases), so it needs no per-family generalization.
