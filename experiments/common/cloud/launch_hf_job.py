...
to experiments/common/cloud/hf_jobs_cell.sh.
...
Example (the Y-lane plumbing smoke):
  python3 experiments/common/cloud/launch_hf_job.py \\
...
def build_command(args, extract_args: list[str]) -> list[str]:
    cell = " ".join([
        f"bash experiments/common/cloud/{args.cell_script}",
...
def build_ai_verdict_command(args) -> list[str]:
    ...
    """
    cell = " ".join([
        "bash experiments/probe-as-reward/cloud/hf_jobs_ai_verdict.sh",
