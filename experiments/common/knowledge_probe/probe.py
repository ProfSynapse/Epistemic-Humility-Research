#!/usr/bin/env python3
"""Knowledge probe runner.

Location: experiments/common/knowledge_probe/probe.py
Reads:    experiments/common/configs/knowledge-probe/probe.yaml (pinned sampling config)
          datasets/triviaqa-rc-nocontext/train.jsonl (WS-0 fetch output)
Writes:   <output.root>/<model_tag>/probe_results.jsonl  (A -> B contract)
          <output.root>/<model_tag>/probe_manifest.json
          <output.root>/<model_tag>/sensitivity_grid.json
...
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "experiments/common/artifacts/knowledge_probe"
...
def main() -> None:
    ...
    default_config = (
        REPO_ROOT / "experiments/common/configs/knowledge-probe/probe.yaml"
    )
    ...
    output_root = Path(config.get("output", {}).get("root", DEFAULT_OUTPUT_ROOT))
    if not output_root.is_absolute():
        output_root = REPO_ROOT / output_root
    out_dir = output_root / config["model"]["model_tag"]
