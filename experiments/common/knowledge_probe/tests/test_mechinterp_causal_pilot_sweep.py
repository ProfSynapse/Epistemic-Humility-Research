...
import causal_pilot_aggregate as aggregate  # noqa: E402
import causal_pilot_sweep as sweep  # noqa: E402
...
def test_checked_in_full_sweep_uses_docker_commands():
    ...
    assert config_arg.startswith(
        "/workspace/repo/archive/experiment/phase1/probe/qwen3-4b-instruct/"
    )
...
def test_docker_runner_config_rewrites_repo_paths_for_container():
    ...
    config = sweep.build_runner_config(
        template_config={
            ...
            "selection": {
                "probe_results": str(
                    repo_root / "archive" / "experiment" / "phase1" / "probe" / "qwen3-4b-instruct" / "probe_results.jsonl"
                ),
            },
            "output": {"root": "old"},
        },
        candidate={
            ...
            "direction_id": "direction__a",
            "direction_file": str(
                repo_root / "archive" / "experiment" / "phase1" / "probe" / "qwen3-4b-instruct" / "hidden_states" / "dir.safetensors"
            ),
            "direction_csv": "archive/experiment/phase1/probe/qwen3-4b-instruct/hidden_states/directions.csv",
            "direction_manifest": str(
                repo_root / "archive" / "experiment" / "phase1" / "probe" / "qwen3-4b-instruct" / "hidden_states" / "manifest.json"
            ),
            "extraction_dir": str(
                repo_root / "archive" / "experiment" / "phase1" / "probe" / "qwen3-4b-instruct" / "hidden_states"
            ),
            "extraction_manifest": "archive/experiment/phase1/probe/qwen3-4b-instruct/hidden_states/extraction_manifest.json",
        },
        output_root=repo_root / "experiments" / "common" / "knowledge_probe" / "out",
    ...
    assert candidate["direction_csv"].startswith("/workspace/repo/archive/")
    assert candidate["direction_manifest"].startswith("/workspace/repo/archive/")
    assert candidate["extraction_dir"].startswith("/workspace/repo/archive/")
    assert candidate["extraction_manifest"].startswith("/workspace/repo/archive/")
