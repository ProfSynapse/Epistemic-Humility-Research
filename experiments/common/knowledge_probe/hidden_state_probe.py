...
PROBE_DIR = REPO_ROOT / "experiments/common/artifacts/knowledge_probe"
...
def resolve_output_dir(config: dict, extraction_config_sha: str,
    ...
    """<output-root>/<model_tag>/hidden_states/<extraction_id>/ (shared per extraction).
...
def select_selfaware_manifest_slice(config: dict) -> list[dict]:
    ...
    This path consumes `mechinterp-selfaware-frozen-row-manifest/v1` rows. It maps
...
def main() -> None:
    ...
    default_config = (
        REPO_ROOT / "experiments/common/configs/knowledge-probe/hidden_state_probe.yaml"
    )
