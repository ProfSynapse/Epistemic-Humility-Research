...
from instrument_common import (
    ...
    require_synaptic_tuner_source,
)
from source_and_generate import (
    BaselineRunLog, _generation_config, _load_vllm_completions,
    _load_vllm_provenance, assign_role, generation_config_sha256,
    validate_source_materialization, validate_structured_completion,
)
...
def validate_generation_inputs(
    ...
    expected_gen_hash = generation_config_sha256(gen)
    expected_scheduler_pins = {
    ...
    expected_loader_pins = {"trust_remote_code": gen["trust_remote_code"]}
    expected_hardware_pins = {
    ...
    prompt_rows = load_jsonl(vllm_root / "prompts_private.jsonl")
    completion_rows = _load_vllm_completions(
        vllm_root / "completions.jsonl", prompt_rows, gen,
    )
    schema = json.loads((vllm_root / "output_schema.json").read_text(encoding="utf-8"))
    schema_hash = generation_config_sha256(schema)
    runtime_provenance = _load_vllm_provenance(
        vllm_root / "provenance.json", gen, schema_hash,
        sha256_file(vllm_root / "prompts_private.jsonl"),
    )
    runtime_hardware = runtime_provenance["runtime"]["hardware"]
    for rec in generations:
        ...
        completion_ids = rec["completion_token_ids"]
        prompt = next(p for p in prompt_rows if p["row_key"] == rec["row_key"])
        prompt_ids = prompt["prompt_token_ids_expected"]
        prompt_ids_sha256 = hashlib.sha256(
            json.dumps(prompt_ids, separators=(",", ":")).encode("ascii")
        ).hexdigest()
        if (
            not isinstance(completion_ids, list)
            or not all(isinstance(v, int) for v in completion_ids)
            or rec["n_new_tokens"] != len(completion_ids)
            or rec["last_completion_token_id"] != (completion_ids[-1] if completion_ids else None)
            or rec["prompt_token_ids_sha256"] != prompt_ids_sha256
            or rec["prompt_token_count"] != len(prompt_ids)
        ):
            raise ValueError(f"generation record {rec['row_key']} has invalid token evidence")
        terminated = rec["finish_reason"] != "length"
        if (
            rec["finish_reason"] not in {"stop", "eos_token", "length"}
            or rec["terminated_naturally"] is not terminated
            or rec["schema_valid"] is not True
            or rec["generation_engine"] != "vllm"
            or rec["generation_engine_version"] != gen["engine_version"]
            or rec["generation_config_sha256"] != expected_gen_hash
            or rec["batch_invariant"] is not gen["batch_invariant"]
            or rec["structured_output_backend"] != gen["structured_output_backend"]
            or rec["structured_output_disable_any_whitespace"]
        ...
        parsed = validate_structured_completion(rec["generation_text"])
        completion = completion_rows[rec["row_key"]]
        if (
            rec["parsed_object"] != parsed
            or rec["prompt_bytes_sha256"] != completion["prompt_sha256"]
            or rec["vllm_version"] != gen["engine_version"]
            or rec["vllm_model_runner"] != gen["model_runner"]
            or rec["image_digest"] != cfg["containers"]["generation"]["image_digest"]
            or rec["runtime_versions"].get("vllm") != gen["engine_version"]
            or rec["runtime_versions"].get("torch") != runtime_hardware["torch_version"]
            or rec["runtime_versions"].get("cuda") != runtime_hardware["cuda_runtime"]
            or not isinstance(rec["runtime_versions"].get("transformers"), str)
            or not rec["runtime_versions"]["transformers"]
            or rec["schema_sha256"] != schema_hash
            or rec["scheduler_pins"] != expected_scheduler_pins
            or rec["loader_pins"] != expected_loader_pins
            or rec["hardware_pins"] != expected_hardware_pins
            or rec["synaptic_tuner_source_fingerprint"] != tuner_source["sha256"]
            or rec["checkpoint_config_sha256"] != runtime_provenance["config_hash"]
        ):
            raise ValueError(f"generation record {rec['row_key']} fails vLLM provenance parity")
...
def stage_row_exhaust(
    ...
    for rec in generations:
        ...
        row.pop("prompt_token_ids", None)
        row.pop("completion_token_ids", None)
...
def build_pool(model_id: str) -> dict[str, Any]:
    ...
    consumed = [
        ...
        ANALYSIS / model_id / "vllm" / "prompts_private.jsonl",
        ANALYSIS / model_id / "vllm" / "completions.jsonl",
        ANALYSIS / model_id / "vllm" / "output_schema.json",
        ANALYSIS / model_id / "vllm" / "provenance.json",
        ANALYSIS / model_id / "vllm" / "resume_history.jsonl",
    ]
