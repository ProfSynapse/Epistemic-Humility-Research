        ...
        "generation": {
            ...
            "vllm": {
                ...
                "per_model": {
                    ...
                    "qwen3_4b_raw_base": {"limit_mm_per_prompt": {}},
                },
...
def test_vllm_completion_contract_rejects_token_or_row_mismatch(tmp_path: Path) -> None:
    ...
    observed = _load_vllm_completions(path, expected, {"max_new_tokens": 200})
    ...
    with pytest.raises(ValueError, match="prompt tokens differ"):
        _load_vllm_completions(path, expected, {"max_new_tokens": 200})
...
def test_presign_selection_is_deterministic_and_spans_registered_strata() -> None:
    ...
    first = select_smoke_rows(candidates, n_rows=20, seed=20260721)
    second = select_smoke_rows(list(reversed(candidates)), n_rows=20, seed=20260721)
