#!/usr/bin/env python3
"""Extract prompt-anchor activations for the fresh multi-source confab rows.

Ported from `j-space-layer-contrast-replication-qwen3-4b/extract_fresh_anchor.py`
(rep1), unchanged except for default paths. The known-correct side is not
extracted here at all -- it is reused verbatim from rep1 via
`materialize_known_side_reuse.py`. This script writes private safetensors
anchor states for hs23/26/29/34 over the fresh confab rows only. It does not
commit question text or activations.
"""
...
DEFAULT_OUT = ANALYSIS / "multisource_confab_anchor_extract.safetensors"
DEFAULT_MANIFEST = ANALYSIS / "multisource_confab_anchor_extract_manifest.json"
...
def run(args: argparse.Namespace) -> int:
    ...
    if not rows:
        print(f"[extract-multisource] ERROR: no rows in {rows_path}", file=sys.stderr)
        return 1
    out_path.parent.mkdir(parents=True, exist_ok=True)

    print(f"[extract-multisource] loading {MODEL_NAME} bf16 unsloth", flush=True)
    ...
    try:
        for idx, row in enumerate(rows, start=1):
            ...
            if idx % 50 == 0 or idx == len(rows):
                print(f"[extract-multisource] {idx}/{len(rows)}", flush=True)
    finally:
        del model
    ...
    manifest = {
        "stage": "j_space_layer_contrast_rep2_multisource_confab_anchor_extract",
