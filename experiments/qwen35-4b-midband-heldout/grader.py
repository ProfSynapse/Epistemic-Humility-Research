"""Grader for qwen35-4b-midband-heldout.

Byte-for-byte port (logic, not import -- each experiment directory owns its
own copy, per this repo's convention) from
`experiments/qwen35-4b-midband-doubt-snap/grader.py`, read in full before
writing this. That module is itself a byte-for-byte port of
`doubt-snap-cross-family-confirmatory/grader.py` -- the self-contained
version (`_normalize_answer_text`/`_is_correct`/`_is_stated_confidence_refusal`
defined locally, no external `scorers` module dependency), which is the
version cell.yaml's `readouts.refused` names explicitly ("ladder grader.py
definition, reused"). `is_degenerate` is the coherence-floor check (empty /
no-alnum / repeated word n-gram / repeated character n-gram).
...
Fields returned by `grade_one`:
  ...
  well_formed_correct bool   answered and (not degenerate) and correct is True
                       -- reported for continuity; NOT this experiment's cost
                       metric (the cost gate uses `refused` directly, per
                       gates.yaml `false_refusal_rate`).
"""
