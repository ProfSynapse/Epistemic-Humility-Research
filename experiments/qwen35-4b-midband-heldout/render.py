"""Cross-family JSON-answer render for the qwen35-4b-midband-heldout cell.

Ported (logic, not cross-experiment import, per this repo's own convention
of each experiment directory owning its own copy) from
`experiments/doubt-snap-cross-family-confirmatory/render.py`, read in full
before writing this -- cell.yaml's `frozen_operating_point.render.source`
names that file as the source verbatim: the prompt contract, anchor
position (prompt_len - 1), and thinking-off self-check are unchanged; only
the environment variable names are namespaced to this experiment
(QW35H_RENDER_MODEL / QW35H_RENDER_REVISION) so this cell's render cache can
never collide with the ladder's or the fleet's if ever imported in the same
process.
"""
...
# Qwen3.5's literal thinking-tag strings (this substrate is the only one
# this experiment ever renders for, but the check is kept as a tripwire
# rather than assumed, matching the ladder's own render.py).
...
def _log_fallback_once(model: str, mode: str, detail: str) -> None:
    ...
    print the same fallback thousands of times per run.
    """
    key = (model, mode)
    if key in _LOGGED_FALLBACKS:
        return
    _LOGGED_FALLBACKS.add(key)
    print(f"[qw35h-render] {model}: {detail}", file=sys.stderr, flush=True)


def _tokenizer():
    global _TOKENIZER, _TOKENIZER_KEY
    model = os.environ.get("QW35H_RENDER_MODEL")
    if not model:
        raise RuntimeError("QW35H_RENDER_MODEL must name the HF tokenizer repo")
    revision = os.environ.get("QW35H_RENDER_REVISION") or None
    key = (model, revision)
...
def render(row: dict) -> str:
    ...
    tok = _tokenizer()
    model = os.environ.get("QW35H_RENDER_MODEL", "<unset>")
