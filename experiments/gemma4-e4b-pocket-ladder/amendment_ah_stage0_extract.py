...
experiment's vendored `scorers.py` (see NOTEBOOK.md "VENDOR SCORERS") and to
`backends.py`, which restores the other half of `model_lib.render()`'s import
chain.

VENDORED INTO THIS EXPERIMENT 2026-07-25, from the parent
`j-space-cross-family-layer-contrast`. One deliberate change from the parent's
copy: `_CONFIG_PATH` was an absolute path into the canonical checkout, which in
a git worktree silently binds the run to a DIFFERENT checkout's copy of the
prompt than the one under review. It now resolves repo-root-relative from this
file. The hash freeze below is what actually guarantees content, so relative
resolution is strictly safer -- it reads the prompt from whichever tree is being
run, and still refuses if that tree's prompt is not the frozen one. Nothing else
is changed; the frozen hash is byte-identical to the parent's.
...
def _config_path() -> Path:
    """Resolve the config repo-root-relative by walking up from this file.

    Deliberately not absolute: this experiment is developed in git worktrees,
    and an absolute path would bind a worktree run to the canonical checkout's
    copy of the prompt rather than the one being reviewed. The sha256 freeze
    below is the real content guarantee either way.
    """
    here = Path(__file__).resolve()
    for candidate in here.parents:
        if (candidate / _CONFIG_REL).is_file():
            return candidate / _CONFIG_REL
    raise RuntimeError(
        f"[amendment_ah_stage0_extract shim] could not locate {_CONFIG_REL} in "
        f"any parent of {here}. This shim restores the baseline system prompt "
        "that every family's generation renders under; without it there is no "
        "verified prompt to render."
    )
...
def load_baseline_system_prompt() -> str:
    ...
    if actual != _EXPECTED_SHA256:
        raise RuntimeError(
            f"[amendment_ah_stage0_extract shim] {config_path} prompt.system "
