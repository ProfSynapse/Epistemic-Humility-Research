...
class BaselineRunLog:
    ...
    REQUIRED = {
        ...
        "suppressed_token_strings", "suppressed_token_ids",
...
def _generation_config(cfg: dict[str, Any], model_id: str) -> dict[str, Any]:
    ...
    if cfg["generation"].get("canonical_eos_enabled") is not True:
        raise ValueError("canonical EOS must remain enabled")
    suppression_pins = vllm_cfg["per_model"][model_id]["suppress_tokens"]
    suppress_token_strings = [pin["token"] for pin in suppression_pins]
    expected_suppressed_token_ids = [int(pin["expected_id"]) for pin in suppression_pins]
    if (
        not suppress_token_strings
        or len(suppress_token_strings) != len(set(suppress_token_strings))
        or len(expected_suppressed_token_ids) != len(set(expected_suppressed_token_ids))
    ):
        raise ValueError(f"{model_id}: suppression pins must be nonempty and distinct")
    return {
        ...
        "suppress_token_strings": suppress_token_strings,
        "expected_suppressed_token_ids": expected_suppressed_token_ids,
...
def validate_suppression_mapping(
    tokenizer: Any, gen: dict[str, Any],
) -> dict[str, Any]:
    """Resolve every suppression pin before any model-generation subprocess."""
    observed_ids: list[int] = []
    for token, expected_id in zip(
        gen["suppress_token_strings"], gen["expected_suppressed_token_ids"]
    ):
        token_ids = tokenizer.encode(token, add_special_tokens=False)
        if token_ids != [expected_id]:
            raise ValueError(
                f"suppression token {token!r} mapped to {token_ids!r}; "
                f"expected exactly [{expected_id}]"
            )
        observed_ids.append(expected_id)
    canonical_eos = tokenizer.eos_token_id
    canonical_eos_ids = (
        list(canonical_eos)
        if isinstance(canonical_eos, (list, tuple, set))
        else [canonical_eos]
    )
    if (
        not canonical_eos_ids
        or any(
            not isinstance(token_id, int) or isinstance(token_id, bool)
            for token_id in canonical_eos_ids
        )
    ):
        raise ValueError("tokenizer canonical eos_token_id is missing or invalid")
    if set(observed_ids).intersection(canonical_eos_ids):
        raise ValueError("suppressed token IDs intersect canonical eos_token_id")
    return {
        "token_strings": list(gen["suppress_token_strings"]),
        "token_ids": observed_ids,
    }


def load_prior_failure_manifest(cfg: dict[str, Any]) -> dict[str, Any]:
    spec = cfg["presign_reachability"]["prior_failure_manifest"]
    relative = Path(spec["artifact"])
    if relative.is_absolute() or ".." in relative.parts:
        raise ValueError("prior failure manifest path must be repository-relative")
    path = ROOT.parents[1] / relative
    if sha256_file(path) != spec["sha256"]:
        raise ValueError("prior failure manifest hash mismatch")
    value = json.loads(path.read_text(encoding="utf-8"))
    failure_ids = value.get("failure_ids")
    expected_count = int(spec["expected_count"])
    if (
        value.get("schema_version") != 1
        or value.get("source_experiment")
        != "family-atlas-surface-matched-vllm-control"
        or value.get("model_id") != "gemma4_e4b_it"
        or value.get("expected_completion_rows")
        != int(cfg["private_predecessor_comparator"]["completions"]["expected_rows"])
        or value.get("expected_strict_valid_rows")
        != int(
            cfg["private_predecessor_comparator"]["completions"]
            ["expected_strict_valid"]
        )
        or value.get("origin_hashes", {}).get("completions_sha256")
        != cfg["private_predecessor_comparator"]["completions"]["sha256"]
        or value.get("failure_count") != expected_count
        or value.get("failure_count")
        != int(cfg["private_predecessor_comparator"]["completions"]["expected_invalid"])
        or value.get("failure_count")
        != value.get("expected_completion_rows") - value.get("expected_strict_valid_rows")
        or not isinstance(failure_ids, list)
        or len(failure_ids) != expected_count
        or len(set(failure_ids)) != expected_count
        or any(not isinstance(row_id, str) or not row_id.startswith("umwp:") for row_id in failure_ids)
    ):
        raise ValueError("prior failure manifest structure or membership is invalid")
    return value


def load_prior_smoke_ids(cfg: dict[str, Any], model_id: str) -> list[str]:
    spec = cfg["presign_reachability"]["base_smoke_manifests"][model_id]
    relative = Path(spec["artifact"])
    if relative.is_absolute() or ".." in relative.parts:
        raise ValueError("prior smoke manifest path must be repository-relative")
    path = ROOT.parents[1] / relative
    if sha256_file(path) != spec["sha256"]:
        raise ValueError(f"{model_id}: prior smoke manifest hash mismatch")
    rows = load_jsonl(path)
    row_ids = [row.get("row_id") for row in rows]
    expected_count = int(spec["expected_count"])
    if (
        len(rows) != expected_count
        or len(set(row_ids)) != expected_count
        or any(not isinstance(row_id, str) for row_id in row_ids)
        or spec["model_only_row_id"] not in row_ids
    ):
        raise ValueError(f"{model_id}: prior smoke manifest membership is invalid")
    return row_ids


def validate_predecessor_parity(
    cfg: dict[str, Any], model_id: str,
    successor: dict[str, dict[str, Any]], *,
    private_path: Path | None = None, committed_path: Path | None = None,
) -> dict[str, Any] | None:
    """Gate the successor against the immediate vLLM predecessor without text parity."""
    spec = cfg["private_predecessor_comparator"]
    if model_id != spec["model_id"]:
        return None
    predecessor_path = Path(spec["completions"]["local_artifact"])
    if sha256_file(predecessor_path) != spec["completions"]["sha256"]:
        raise ValueError("private predecessor completion hash mismatch")
    predecessor_rows = load_jsonl(predecessor_path)
    by_id = {str(row.get("id", "")): row for row in predecessor_rows}
    expected_rows = int(spec["completions"]["expected_rows"])
    if len(predecessor_rows) != expected_rows or len(by_id) != expected_rows or "" in by_id:
        raise ValueError("private predecessor completion coverage is invalid")
    if set(by_id) != set(successor):
        raise ValueError("successor and predecessor completion IDs differ")

    failure_manifest = load_prior_failure_manifest(cfg)
    failure_ids = set(failure_manifest["failure_ids"])
    observed_invalid: set[str] = set()
    parsed_predecessor: dict[str, dict[str, Any]] = {}
    for row_id, row in by_id.items():
        try:
            parsed_predecessor[row_id] = validate_structured_completion(
                row.get("completion_text")
            )
        except ValueError:
            observed_invalid.add(row_id)
    expected_valid = int(spec["completions"]["expected_strict_valid"])
    if (
        len(parsed_predecessor) != expected_valid
        or len(observed_invalid) != int(spec["completions"]["expected_invalid"])
        or observed_invalid != failure_ids
    ):
        raise ValueError("private predecessor strict-valid partition is not registered")

    records: list[dict[str, Any]] = []
    failures: list[str] = []
    cap = int(cfg["generation"]["max_new_tokens"])
    for row_id in sorted(by_id):
        old = by_id[row_id]
        new = successor[row_id]
        successor_valid = True
        try:
            successor_parsed = validate_structured_completion(new.get("completion_text"))
        except ValueError:
            successor_valid = False
            successor_parsed = None
        is_prior_failure = row_id in failure_ids
        token_ids_exact = (
            old.get("completion_token_ids") == new.get("completion_token_ids")
            if not is_prior_failure else None
        )
        finish_reason_exact = (
            old.get("finish_reason") == new.get("finish_reason")
            if not is_prior_failure else None
        )
        parsed_object_exact = (
            parsed_predecessor[row_id] == successor_parsed
            if not is_prior_failure and successor_valid else False
        ) if not is_prior_failure else None
        successor_natural_before_cap = (
            new.get("finish_reason") != "length"
            and isinstance(new.get("completion_token_ids"), list)
            and len(new["completion_token_ids"]) < cap
        )
        row_pass = (
            successor_valid and successor_natural_before_cap
            if is_prior_failure
            else bool(token_ids_exact and finish_reason_exact and parsed_object_exact)
        )
        records.append({
            "row_id": row_id,
            "predecessor_strict_valid": not is_prior_failure,
            "successor_strict_valid": successor_valid,
            "completion_token_ids_exact": token_ids_exact,
            "finish_reason_exact": finish_reason_exact,
            "parsed_object_exact": parsed_object_exact,
            "successor_natural_before_cap": successor_natural_before_cap,
            "parity_requirement_pass": row_pass,
        })
        if not row_pass:
            failures.append(row_id)

    private_path = private_path or (
        ANALYSIS / model_id / "predecessor_parity_private.jsonl"
    )
    committed_path = committed_path or (
        COMMITTED / model_id / "g0_predecessor_parity_summary.json"
    )
    ...
    summary = {
        ...
        "model_id": model_id,
        "n_rows": len(records),
        "n_predecessor_strict_valid": len(parsed_predecessor),
        "n_prior_failures_repaired": sum(
            record["parity_requirement_pass"] and not record["predecessor_strict_valid"]
            for record in records
        ),
        "n_parity_failures": len(failures),
        "predecessor_completions_sha256": spec["completions"]["sha256"],
        "failure_manifest_sha256": cfg["presign_reachability"]
        ["prior_failure_manifest"]["sha256"],
        "private_evidence_sha256": sha256_file(private_path),
    }
    atomic_json(committed_path, summary)
    if failures:
        raise ValueError(
            f"predecessor parity G0 failed for {len(failures)} rows"
        )
    return summary
...
def build_vllm_command(
    ...
    for token in gen["suppress_token_strings"]:
        command.extend(["--suppress-token", token])
...
def _load_vllm_completions(
    ...
    for row_id, item in by_id.items():
        ...
        suppressed = set(gen["expected_suppressed_token_ids"])
        if suppressed.intersection(completion_ids):
            raise ValueError(f"{row_id}: completion contains a suppressed token ID")
...
def _load_vllm_provenance(
    ...
    expected = {
        ...
        "suppress_tokens": gen["suppress_token_strings"],
    }
    ...
    if (
        ...
        or runtime.get("suppress_tokens") != gen["suppress_token_strings"]
        or runtime.get("suppressed_token_ids")
        != gen["expected_suppressed_token_ids"]
        or runtime.get("suppressed_bad_word_token_ids")
        != [[token_id] for token_id in gen["expected_suppressed_token_ids"]]
...
def run_generation(model_id: str, rows_path: Path) -> None:
    ...
    validate_predecessor_parity(cfg, model_id, completions)
    ...
    for row in rows:
        ...
        log.append({
            ...
            "suppressed_token_strings": suppression_validation["token_strings"],
            "suppressed_token_ids": suppression_validation["token_ids"],
