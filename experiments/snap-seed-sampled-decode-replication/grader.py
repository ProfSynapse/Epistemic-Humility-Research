"""Grader, reused for H3 snap-seed-sampled-decode-replication.

Copied verbatim (byte-identical, not imported cross-experiment per this
repo's own convention) from
`experiments/doubt-gated-caution-tighten/grader.py` -- AMENDMENT.md's Design
requires reusing that cell's grading unchanged. Do not edit the logic here
without also updating the resolved cell's copy.

That module's own docstring (preserved below) documents its further
provenance: ported (logic, not import, since the source lives on an unmerged
sibling branch) from `two-signal-caution-regulation-instruct/two_signal_grader.py`
(worktree `/home/profsynapse/code/ehr-worktrees/two-signal`, branch
`exp/two-signal-caution-regulation-instruct`, read in full before writing
this): `is_degenerate` is the coherence-floor check (empty / no-alnum /
repeated word n-gram / repeated character n-gram, including the JSON-answer-
field-aware and no-whitespace-loop fixes that source module's own docstring
documents finding necessary on this exact surface's real generations), and
`grade_one` adds `well_formed_correct` for the known-correct cost-control
population.
