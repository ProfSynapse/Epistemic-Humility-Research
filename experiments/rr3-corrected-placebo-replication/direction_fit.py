"""Direction/gate fit module for rr3-corrected-placebo-replication.

Byte-for-byte port (except this docstring) of
`experiments/rr2-mistral-adjudicated-refusal-confirm/direction_fit.py`,
itself a verbatim port of
`experiments/rr-cross-family-raw-refusal/direction_fit.py`. This experiment
runs NO fresh FIT stage for either family (AMENDMENT.md "Design": "No fresh
FIT stage and no selection freedom" for the mistral core; "No fresh FIT" for
the llama rider). This module is reused ONLY by `fit_reuse.py`, which calls
`fit_directions`/`fit_byte_identical`/`fit_gate` to RECONSTRUCT RR's
already-committed hs16 (mistral) and hs20 (llama) fits deterministically from
the same FIT rows and anchors RR used, then asserts the reconstruction's
stats match RR's committed manifests field-for-field (RG0 hard stop).

Per cell.yaml `core_cell.instrument_write` / `rider_cells.rider_llama_placebo_ladder`:
...
written (`fit_byte_identical`, RG0 `directions_byte_identical` check).
"""
...
def fit_gate(fit: dict[str, Any]) -> dict[str, Any]:
    ...
    freeze tau via Youden-J. Returns the gate summary for RG0's
    fit-reconstruction cross-check and later fire-decision scoring."""
    stats = fit["stats"]
