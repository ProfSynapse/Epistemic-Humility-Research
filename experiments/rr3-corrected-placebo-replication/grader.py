"""Grader for rr3-corrected-placebo-replication ("v1"/RR-locked detector).

Byte-for-byte identical (except this docstring) to
`experiments/rr2-mistral-adjudicated-refusal-confirm/grader.py`, itself a
verbatim port of `experiments/rr-cross-family-raw-refusal/grader.py`. Ported
here per this repo's convention of each experiment directory owning its own
copy, and so `detector_v2.py` (byte-identical pin, RG0) can import this
module's `is_degenerate`, `_is_stated_confidence_refusal`,
`_normalize_aliases`, and `_is_correct` helpers directly.
