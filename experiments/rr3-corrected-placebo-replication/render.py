"""JSON-answer render for rr3-corrected-placebo-replication.

Ported (logic, not cross-experiment import, per this repo's convention of
each experiment directory owning its own copy) from
`experiments/rr2-mistral-adjudicated-refusal-confirm/render.py`, read in full
before writing this. Prompt contract, anchor position, and thinking-off
self-check are unchanged; only the environment variable names are namespaced
to THIS experiment (RR3_RENDER_MODEL / RR3_RENDER_REVISION) so this render
cache can never collide with RR2's own render module
(RR2_RENDER_MODEL/RR2_RENDER_REVISION) or RR's
(RR_RENDER_MODEL/RR_RENDER_REVISION) if ever imported in the same process --
the same namespacing rationale one level up. This module is family-agnostic:
`load_model` (steer_lib.py) sets these env vars to whichever family/revision
is currently loaded, so the SAME render.py serves both the mistral core cell
and the llama rider cell, never simultaneously in one process.
"""
...
# Neither Mistral-7B-Instruct-v0.3 nor Llama-3.2-3B-Instruct emit thinking
# tags, so this check is a no-op on both this experiment's substrates; kept
# for parity with the ported source and as a tripwire if this harness is
# ever pointed at a thinking-capable model.
...
def _log_fallback_once(model: str, mode: str, detail: str) -> None:
    ...
    print(f"[rr3-render] {model}: {detail}", file=sys.stderr, flush=True)


def _tokenizer():
    global _TOKENIZER, _TOKENIZER_KEY
    model = os.environ.get("RR3_RENDER_MODEL")
    if not model:
        raise RuntimeError("RR3_RENDER_MODEL must name the HF tokenizer repo")
    revision = os.environ.get("RR3_RENDER_REVISION") or None
    key = (model, revision)
...
def render(row: dict) -> str:
    ...
    tok = _tokenizer()
    model = os.environ.get("RR3_RENDER_MODEL", "<unset>")
