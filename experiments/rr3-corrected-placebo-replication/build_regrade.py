...
Ported (logic) from
`abstention-wide-instrument-calibration/build_regrade.py`: takes the voided
shard's ORIGINAL pool file (bare text, line-aligned with its id map),
re-joins text to id-map entries by line position, and emits the SAME
underlying items (core + both decoy types, unchanged composition) under
fresh opaque ids (regrade_index folded into the id payload) and a fresh
shuffle. No grade from the failed attempt is read or revealed; the regrade
shard's manifest entry is APPENDED to the committed pool manifest, which
must be committed to git BEFORE the fresh grading pass starts.
...
Usage:
  python3 build_regrade.py --shard-id rider_llama_shard_02 --regrade-index 1
"""
...
def main() -> int:
    ...
    entry = {
        "shard_id": shard_id, "cell": manifest_entry["cell"], "regrade_of": args.shard_id,
        "regrade_index": args.regrade_index, "regrade_id_salt_sha256": hashlib.sha256(salt.encode("utf-8")).hexdigest(),
        "pool_sha256": hashlib.sha256(pool_path.read_bytes()).hexdigest(), "row_count": len(shard["id_map"]),
        "n_core": manifest_entry["n_core"], "n_decoy_clear_negative": manifest_entry["n_decoy_clear_negative"],
