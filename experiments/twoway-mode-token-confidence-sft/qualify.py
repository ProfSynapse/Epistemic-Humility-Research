#!/usr/bin/env python3
"""Prepare, generate, resume, and adjudicate the two-way dev qualification.

Copy-and-modify of ``fresh-sft-epistemic-mode-token-grpo/qualify_stage_s.py`` for
the two-mode successor. Differences from Stage-S:

- MODE_NAMES has two entries (ANSWER, ABSTAIN); forced-posture spans two tokens.
- runtime token lineage is read from ``training.yaml`` and must be exactly the two
  configured strings.
- the answer-quality gate is the rethought one (AMENDMENT.md sec 6.6): a paired
  correctness bootstrap restricted to rows the policy chooses to ANSWER, plus an
  abstention-rate band.
- a first-class confidence calibration gate is added (sec 6.7): discrimination
  AUROC and ECE on attempted rows.

Model execution crosses only the pinned Synaptic-Tuner public ``batch-generate``
CLI (tuner commit f6f1229+, the PEFT-aware adapter-dir loader). This module owns
prompt construction and scoring; it never imports tuner internals and never opens
the sealed held-out row file.
"""
...
def hash_artifact_tree(root: Path) -> dict[str, Any]:
    root = root.resolve()
    if not root.is_dir():
        raise QualificationError(f"stage artifact tree is missing: {root}")
    entries: list[dict[str, Any]] = []
    for path in sorted(root.rglob("*")):
        if path.is_symlink():
            raise QualificationError(f"stage artifact tree contains a symlink: {path}")
        if path.is_file():
            entries.append(
                {
    ...
    if not entries:
        raise QualificationError("stage artifact tree contains no files")
    return {
        "root": str(root),
        "files": entries,
...
def verify_tuner(config: Mapping[str, Any], tuner_root: Path) -> str:
    """Require the tuner HEAD to be exactly the pinned repaired-loader commit."""
    ...
    # Accept an exact match or an abbreviated pinned prefix (e.g. f6f1229),
    # since the repaired loader is pinned by its short SHA in the amendment.
...
def load_contract(config_path: Path) -> tuple[dict[str, Any], Path, Path]:
    ...
    if "PENDING_" in yaml.safe_dump(config):
...
def load_dev_rows(
    ...
    forbidden = _resolve_within(
        repo_root, str(config["heldout"]["path_forbidden_in_runner"]), "heldout path"
    )
    ...
    if sha256_file(dev_path) != config["dev"]["sha256"]:
    ...
    for row in rows:
        ...
        if metadata.get("mode_label") not in MODE_NAMES:
            raise QualificationError("dev row has a non-two-way mode label")
    return rows
...
def load_runtime_tokens(
    ...
    training_config = _load_yaml(
        _resolve_within(experiment_dir, config["training_config"], "training_config")
    )
    configured = list(training_config["model"]["tokenizer"]["additional_special_tokens"])
    if len(configured) != len(MODE_NAMES) or len(set(configured)) != len(configured):
        raise QualificationError("configured token strings must be two unique values")
    ...
    if lineage_tokens != configured:
        raise QualificationError("runtime token strings/order differ from training.yaml")
...
def prepare_run(
    ...
    *,
    ...
    stage_model = stage_model.resolve()
    lineage_path = lineage_path.resolve()
    try:
        lineage_relative = lineage_path.relative_to(stage_model)
    except ValueError as exc:
        raise QualificationError("special-token lineage must be inside the stage artifact tree") from exc
    ...
    if tokenizer is None:
        ...
        tokenizer = AutoTokenizer.from_pretrained(str(stage_model), local_files_only=True)
    tokens, token_ids, lineage = load_runtime_tokens(config, experiment_dir, lineage_path, tokenizer)
    artifact_tree = hash_artifact_tree(stage_model)
    ...
    prompt_paths = {key: run_dir / config["output"]["prompt_files"][key] for key, _ in prompt_records}
    prompt_manifest = {
        key: {
    ...
    manifest = {
        ...
        "expected_generation_invocations": {
            ...
            "stage_s_native": {"model": str(stage_model), "revision": None},
            "stage_s_forced": {"model": str(stage_model), "revision": None},
        },
    }
    invariant_keys = (
        "schema_version", "experiment", "run_id", "config_sha256", "dev_sha256", "dev_rows",
        "heldout_rows_accessed", "stage_s_model", "stage_s_artifact_tree",
        "special_token_lineage_path", "special_token_lineage_sha256", "configured_tokens",
        "base_model_provenance", "prompt_files", "expected_generation_invocations",
    )
    if resume:
...
def _generation_command(
    tuner_root: Path, prompts: Path, out_dir: Path, model: str, config: Mapping[str, Any],
    *, revision: str | None, resume: bool,
) -> list[str]:
    generation = config["generation"]
    command = [
        sys.executable, str(tuner_root / "tuner.py"), str(generation["public_tuner_verb"]),
        "--prompts", str(prompts), "--model", model, "--out-dir", str(out_dir),
        "--engine", str(generation["engine"]), "--batch-size", str(generation["batch_size"]),
        "--max-new-tokens", str(generation["max_new_tokens"]),
        "--min-new-tokens", str(generation["min_new_tokens"]), "--seed", str(generation["seed"]),
    ]
    if revision:
...
def generate(
    config_path: Path, run_id: str, stage_model: Path, tuner_root: Path, *, resume: bool
) -> dict[str, Any]:
    ...
    if str(stage_model.resolve()) != manifest.get("stage_s_model"):
        raise QualificationError("generation stage model path differs from prepared manifest")
    if hash_artifact_tree(stage_model) != manifest.get("stage_s_artifact_tree"):
        raise QualificationError("stage artifact tree changed after prepare")
    _verify_prompt_files(run_dir, manifest)
    jobs = (
        ("base_native", str(config["models"]["base"]["repo"]), str(config["models"]["base"]["revision"])),
        ("stage_s_native", str(stage_model.resolve()), None),
        ("stage_s_forced", str(stage_model.resolve()), None),
    )
...
def discrimination_auroc(scores: Sequence[float], labels: Sequence[bool]) -> float | None:
    """Rank-based AUROC (Mann-Whitney). None if a class is empty (undefined)."""
    ...
    while index < len(order):
        j = index
        while j + 1 < len(order) and scores[order[j + 1]] == scores[order[index]]:
            j += 1
...
def expected_calibration_error(
    confidences: Sequence[float], labels: Sequence[bool], buckets: int
) -> float:
    """Equal-width binning ECE of stated confidence vs empirical correctness."""
    if not confidences:
        raise QualificationError("ECE requires at least one attempted row")
    n = len(confidences)
    total = 0.0
    for b in range(buckets):
        ...
        hi = (b + 1) / buckets
        if b == buckets - 1:
            members = [i for i, c in enumerate(confidences) if lo <= c <= hi]
        else:
            members = [i for i, c in enumerate(confidences) if lo <= c < hi]
        if not members:
            continue
...
def _load_scorer(config: Mapping[str, Any], repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("twoway_canonical_scorer", path)
...
def _posture_ok(
    mode: str, payload: dict[str, Any] | None, parse_subgrade: Mapping[str, Any],
...
def _forced_visible_text(completion_text: Any, tokens_by_mode: Mapping[str, str]) -> tuple[str, bool]:
...
def score(config_path: Path, run_id: str, *, resume: bool) -> dict[str, Any]:
    ...
    if hash_artifact_tree(Path(manifest["stage_s_model"])) != manifest.get("stage_s_artifact_tree"):
        raise QualificationError("stage artifact tree changed before scoring")
    ...
    expected_forced = {f"{i}::forced::{m}" for i in expected_ids for m in MODE_NAMES}
    ...
    with scored_path.open("a", encoding="utf-8") as handle:
        for row in rows:
            ...
            native_conf = (
                float(payload["answer_confidence"])
                if isinstance(payload, dict) and native_subgrade["confidence_valid"]
                else None
            )
            ...
            for mode in MODE_NAMES:
                ...
                forced_answer = forced_payload.get("answer") if isinstance(forced_payload, dict) else ""
    ...
    n = len(scored_rows)
    configured = sum(r["stage_s_native"]["configured_first_token"] for r in scored_rows)
    json_valid = sum(r["stage_s_native"]["subgrade"]["json_parse"] for r in scored_rows)
    exact_fields = sum(r["stage_s_native"]["subgrade"]["exact_fields"] for r in scored_rows)
    confidence_valid = sum(r["stage_s_native"]["subgrade"]["confidence_valid"] for r in scored_rows)
    confidence_values = [
        float(r["stage_s_native"]["payload"]["answer_confidence"])
        for r in scored_rows
        if r["stage_s_native"]["subgrade"]["confidence_valid"]
    ]
    confidence_sd = statistics.pstdev(confidence_values) if len(confidence_values) >= 2 else 0.0
    forced_flags = [
        forced_row["posture_compliant"]
        for r in scored_rows
        for forced_row in r["stage_s_forced"].values()
    ]
    native_strip_flags = [r["stage_s_native"]["visible_special_tokens_stripped"] for r in scored_rows]
    forced_strip_flags = [
        forced_row["visible_special_tokens_stripped"]
        for r in scored_rows
        for forced_row in r["stage_s_forced"].values()
    ]
    predicted_counts = Counter(r["stage_s_native"]["predicted_mode"] for r in scored_rows)

    per_mode: dict[str, Any] = {}
    for mode in MODE_NAMES:
        subset = [r for r in scored_rows if r["source_mode"] == mode]
        successes = sum(r["stage_s_native"]["predicted_mode"] == mode for r in subset)
        lower = wilson_lower(successes, len(subset))
        per_mode[mode] = {
            "successes": successes,
            "n": len(subset),
            "recall": successes / len(subset) if subset else 0.0,
            "wilson_lower": lower,
            "pass": lower > float(thresholds["per_mode_recall"]["lower_bound_gt"]),
        }

    # Gate 6a: attempted-row paired correctness (rows the policy chooses to ANSWER).
    attempted = [r for r in scored_rows if r["stage_s_native"]["predicted_mode"] == "ANSWER"]
    noninf = thresholds["answer_quality_noninferiority"]
    attempted_deltas = [
        int(r["stage_s_native"]["correct"]) - int(r["base"]["correct"]) for r in attempted
    ]
    if attempted_deltas:
        att_low, att_high = paired_bootstrap_ci(
            attempted_deltas, int(noninf["seed"]), int(noninf["resamples"])
        )
    else:
        att_low, att_high = float("nan"), float("nan")

    # Gate 6b: abstention-rate band.
    abstain_count = predicted_counts.get("ABSTAIN", 0)
    abstention_rate = abstain_count / n
    band = thresholds["abstention_rate_band"]
    abstention_in_band = float(band["lower"]) <= abstention_rate <= float(band["upper"])

    # Gate 8: confidence calibration on attempted rows (native ANSWER + valid conf).
    cal = thresholds["confidence_calibration"]
    calib_rows = [
        r for r in attempted
        if r["stage_s_native"]["subgrade"]["confidence_valid"]
    ]
    calib_conf = [float(r["stage_s_native"]["payload"]["answer_confidence"]) for r in calib_rows]
    calib_labels = [bool(r["stage_s_native"]["correct"]) for r in calib_rows]
    auroc = discrimination_auroc(calib_conf, calib_labels) if calib_conf else None
    ece = expected_calibration_error(calib_conf, calib_labels, int(cal["ece_buckets"])) if calib_conf else None
    ...
    gate_results = {
        ...
        "per_mode_wilson_majority": all(v["pass"] for v in per_mode.values()),
        "max_single_mode_share": max(predicted_counts.values(), default=0) <= int(thresholds["max_single_mode_share"]["numerator_max"]),
        "attempted_row_noninferiority": bool(attempted_deltas) and att_low > float(noninf["lower_bound_gt"]),
        "abstention_rate_within_band": abstention_in_band,
        "confidence_discrimination_auroc": auroc is not None and auroc >= float(cal["discrimination_auroc_min"]),
        "confidence_ece": ece is not None and ece < float(cal["ece_max"]),
    }
    summary = {
        "schema_version": 1,
        "experiment": config["experiment"],
        ...
        "answer_quality": {
            ...
            "point_difference": (sum(attempted_deltas) / len(attempted_deltas)) if attempted_deltas else None,
            "ci_lower": att_low,
            "ci_upper": att_high,
            "seed": noninf["seed"],
            "resamples": noninf["resamples"],
        },
        "abstention": {
            "count": abstain_count,
            "rate": abstention_rate,
            "band": [float(band["lower"]), float(band["upper"])],
            "within_band": abstention_in_band,
        },
        "confidence_calibration": {
            "attempted_rows_scored": len(calib_rows),
            "discrimination_auroc": auroc,
            "ece": ece,
            "ece_buckets": int(cal["ece_buckets"]),
        },
        "gates": gate_results,
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--stage-model", type=Path, required=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command in {"prepare", "run"}:
            ...
            result = prepare_run(
                args.config, args.run_id, args.stage_model, args.special_token_lineage,
                resume=args.resume,
            )
        if args.command in {"generate", "run"}:
            ...
            result = generate(
                args.config, args.run_id, args.stage_model, args.tuner_worktree, resume=args.resume
            )
