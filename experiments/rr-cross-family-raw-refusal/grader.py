"""Grader for the rr-cross-family-raw-refusal cell.

Ported (logic, not cross-experiment import) from
`experiments/doubt-snap-cross-family-confirmatory/grader.py`, read in full
before writing this: `is_degenerate` is the coherence-floor check and
`grade_one` returns the format-agnostic `refused` flag that is this
experiment's PRIMARY readout (AMENDMENT.md "Why the primary metric is raw
refusal, not clean_tighten"), plus `well_formed_correct` for the
known-correct cost population. Byte-for-byte identical to the source except
for this module's own docstring and the removal of the tuner `grade()`
adapter, which this harness does not use (see NOTEBOOK.md execution-model
adjudication: direct InterventionHook/GenerationInterventionController/RunLog
driving, not the declarative `mechinterp steer` recipe path).
