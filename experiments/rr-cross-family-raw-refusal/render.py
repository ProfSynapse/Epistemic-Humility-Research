"""Cross-family JSON-answer render for the rr-cross-family-raw-refusal cell.

Ported (logic, not cross-experiment import, per this repo's convention of
each experiment directory owning its own copy -- see
doubt-snap-cross-family-confirmatory/render.py's own docstring for the same
pattern one level up) from
`experiments/doubt-snap-cross-family-confirmatory/render.py`, read in full
before writing this. cell.yaml's `instrument.render.source` names that file
as the source verbatim: the prompt contract, anchor position, and
thinking-off self-check are unchanged; only the environment variable names
are namespaced to this experiment (RR_RENDER_MODEL / RR_RENDER_REVISION)
so the two experiments' render caches can never collide if ever imported in
the same process.
"""
...
# Neither Llama-3.2-3B-Instruct nor Mistral-7B-Instruct-v0.3 emit thinking
# tags, so this check is a no-op on both RR substrates; kept for parity with
# the ported source and as a tripwire if a future family extension does emit
# them.
...
def _log_fallback_once(model: str, mode: str, detail: str) -> None:
    ...
    print(f"[rr-render] {model}: {detail}", file=sys.stderr, flush=True)


def _tokenizer():
    global _TOKENIZER, _TOKENIZER_KEY
    model = os.environ.get("RR_RENDER_MODEL")
    if not model:
        raise RuntimeError("RR_RENDER_MODEL must name the HF tokenizer repo")
    revision = os.environ.get("RR_RENDER_REVISION") or None
    key = (model, revision)
...
def render(row: dict) -> str:
    ...
    tok = _tokenizer()
    model = os.environ.get("RR_RENDER_MODEL", "<unset>")
