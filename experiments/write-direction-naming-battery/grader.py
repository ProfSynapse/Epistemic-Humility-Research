"""Grader for write-direction-naming-battery.

Byte-for-byte port (logic, not import -- each experiment directory owns its
own copy, per this repo's convention) from
`experiments/margin-mapping/harness/grader.py`, read in full before writing
this. That module is itself byte-identical to the RR-locked "v1" detector
lineage. This is the lineage cell.yaml `execution.graders` names as
`grader:grade` (narrow semantic_refuse + correctness) and the amendment's
disclosures D-1 through D-4 are computed against
(`experiments/margin-mapping/analysis/runlog/qwen35_4b__*.jsonl` rows carry
exactly these fields), so this port -- not doubt-snap-cross-family's own
grader.py -- is the correct lineage for this cell.
