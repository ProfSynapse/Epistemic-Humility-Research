...
Adapted from `qwen35-4b-midband-heldout/steer_lib.py` (read in full before
writing this): same InterventionHook/GenerationInterventionController/RunLog
driving, same left-padding rationale, same render-env-setting-at-load-site
discipline. Differs from the heldout cell only where the amendment's design
requires it:

  - `run_batch_fixed`/`build_hook_and_controller` take an explicit `sigma`
    per call (this cell doses BOTH c_hat, sigma=sigma_c, AND
    random_direction, sigma=1.0, across MANY multipliers, not one frozen
    dose), so nothing here hard-codes a single dose or a single direction.
  - `run_rows` applies `execution.redact_fields` (cell.yaml: question,
    aliases, answer_text, answer_value) to every record before it is
    persisted -- stricter than the heldout cell's own runlog (which keeps
    answer_text), per this cell's explicit pin. Grading still runs on the
    full un-redacted text; only the persisted record drops those fields.
  - Negative multipliers are dosed by passing a negative `strength` to
    `begin_pass` -- the erase_write law is well-defined for a negative
    setpoint (AMENDMENT.md "Negative dose is well defined under this law").
"""
...
def load_model(model_name: str = MODEL_NAME, revision: str = MODEL_REVISION):
    """Loads the generation model/tokenizer under base conda (cell.yaml
    `execution.python`: the qwen3_5 architecture needs transformers >= 5.x,
    which the project's pinned unsloth_env, transformers 4.57.1, does not
    recognize), AND sets render.py's WDNB_RENDER_MODEL/WDNB_RENDER_REVISION
    to the SAME model/revision in the same place, so the two can never drift
    apart. Left padding keeps every row's real last prompt token in the same
    ...
    os.environ["WDNB_RENDER_MODEL"] = model_name
    os.environ["WDNB_RENDER_REVISION"] = revision or ""
...
def run_batch_fixed(
    ...
    zero-gain hook call: `doubt-snap`'s own run_dose_ladder.py and
    `qwen35-4b-midband-heldout`'s steer_lib.py both register the hook only
    for non-baseline passes). `strength` is a SCALAR gain in sigma units;
    negative values are valid (erase_write's negative-setpoint case)."""
    import torch
...
def run_rows(
    ...
    *,
    ...
    (full gen_lib.grade_row dict, data-exhaust principle), then REDACTS
    question/aliases/answer_text/answer_value (cell.yaml
    `execution.redact_fields`) before RunLog-persisting. Resumable:
