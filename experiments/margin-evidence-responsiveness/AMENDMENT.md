...
- **Channel 2, margin lengthening** (single-dose survival, Option A): for
  ...
  no_answer_baseline survival is tautologically zero (the tipping dose is
  where the baseline abstains), so it needs no generation.

Three arms: no_answer_baseline (recaptured fresh in-regime, not reused from
M2), true_answer (gold answer injected in-context), false_answer_placebo (a
within-dataset category-matched distractor). All arms captured and generated
in one self-consistent batching regime with pinned batch composition. Total
new model passes: 2280 captures + 616 generations = 2896, on the local 3090.
...
1. **Collapse floor** (DERIVED default, TO-CONFIRM): leg 1 floor = 0.5 x the
   ...
   derivation offered 0.25/0.5/0.75/1.0 x gap; 0.5 is the recommended
   "meaningfully toward the known regime" bar matching criterion (d)'s
   qualitative wording. The gap is estimated tightly (Cohen's d 3.34, AUROC
   0.982), so baseline-gap noise is not the binding constraint; the specificity
   leg guards the real risk (a shift from any appended text rather than the
   true answer).
...
4. **Margin-lengthening design** (DERIVED population + JUDGMENT option):
   ...
   616 generations) over Option B full-ladder re-run (6160 generations, 10x),
   because re-laddering reintroduces the M1b batch-composition tipping noise
   at every rung for no benefit at the one dose that matters.
5. **Margin floor** (DERIVED default, TO-CONFIRM): D2 absolute floor 0.056,
   the Wilson 95% half-width at worst-case p=0.5, n=308 (an unpaired anchor;
   the paired design has strictly smaller variance, so a real effect is
   comfortably resolvable), plus the CI-excludes-zero condition.
...
Registered at sign, after design-info disclosure (baseline gap and floors).
