...
def run(args: argparse.Namespace) -> int:
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        with torch.no_grad():
            # use_cache MUST be True. On models that share K/V across layers this
            # is not a performance knob, it is a correctness requirement.
            # gemma-4-E4B blocks 24-41 read donor K/V from blocks 22/23 THROUGH
            # the cache object; with use_cache=False those blocks are starved and
            # every hidden state from hs25 up is garbage (hs00-hs24 stay
            # bit-identical, hs25-hs42 cos 0.732 -> 0.075 vs the correct values),
            # while `.generate()` -- which defaults use_cache=True -- is fine.
            # That silently invalidated the entire first gemma extraction.
            # llama / mistral / qwen are unaffected (min cos 1.000000 either way),
            # which is exactly why this went unnoticed: CPU and GPU agreed at
            # cos 0.998-0.9998 because BOTH ran the broken path -- that agreement
            # is VACUOUS as evidence of faithfulness.
            # This experiment is ABOUT the KV-sharing seam, so a starved-seam
            # forward would confound the very thing it measures.
            # See j-space-cross-family-layer-contrast AMENDMENT.md, "Registered-
            # instrument defects found at resolve", Defect 3.
