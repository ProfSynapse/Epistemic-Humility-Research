...
ROOT = REPO_ROOT / "experiments" / "doubt-snap-cross-family-confirmatory"
...
def run_batch_parity_smoke(cell: dict[str, Any], reference_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in smoke_rows:
        ...
        seq_text = seq_row.get("completion_text", "")
        seq_terminated = seq_row.get("finish_reason") != "length"
        seq_semantic = grader.grade_one(seq_text, row.get("aliases"))
        seq_clean = gen_lib.grade_clean_tighten(seq_text, seq_terminated)
        base_semantic = row.get("baseline_old_grade") or {}
        base_clean = row.get("baseline_clean") or {}
        role_fields_agree = (
            base_semantic.get("well_formed_correct") == seq_semantic.get("well_formed_correct")
            and base_semantic.get("refused") == seq_semantic.get("refused")
            and base_semantic.get("answered") == seq_semantic.get("answered")
            and base_clean.get("clean_tighten") == seq_clean.get("clean_tighten")
        )
        if not role_fields_agree:
...
def prepare(args: argparse.Namespace) -> None:
    ...
    # Termination limb re-specified per AMENDMENT.md signed revision 2: a
    # split row counts as captured iff it emitted EOS naturally, OR its
    # clean grading shows a complete well-formed first-JSON answer and the
    # row is not degenerate (Gemma sometimes appends post-answer prose past
    # the answer object without emitting EOS). The raw EOS-emission rate is
    # retained under its original name plus an explicit alias so neither
    # signed revision drops a field silently.
    eos_emission_rate = sum(1 for r in split_rows if r["baseline_terminated_naturally"]) / max(1, len(split_rows))
    n_captured = sum(
        1
        for r in split_rows
        if r["baseline_terminated_naturally"]
        or (r["baseline_clean"].get("well_formed") and not r["baseline_clean"].get("degenerate"))
    )
    answer_capture_rate = n_captured / max(1, len(split_rows))
    g0 = {
        "counts": counts,
        "held_out_power": counts["confab"]["held_out"] >= 150 and counts["known_correct_answered"]["held_out"] >= 250,
        "generation_terminates_rate": eos_emission_rate,
        "eos_emission_rate": eos_emission_rate,
        "answer_capture_rate": answer_capture_rate,
        "answer_capture_pass": answer_capture_rate >= 0.90,
