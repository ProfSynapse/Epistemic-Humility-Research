"""Grader for placebo-seed-distribution-census ("v1"/RR-locked detector).

Byte-for-byte identical (except this docstring) to
`experiments/rr3-corrected-placebo-replication/grader.py`, itself a verbatim
port of RR2's/RR's own copy. Ported here per this repo's convention of each
experiment directory owning its own copy, and so `detector_v2.py`
(byte-identical pin, SC0) can import this module's `is_degenerate`,
`_is_stated_confidence_refusal`, `_normalize_aliases`, and `_is_correct`
helpers directly.
