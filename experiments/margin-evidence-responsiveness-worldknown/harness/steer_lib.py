"""Batched generation + activation-write driver for margin-mapping (M1).

Direct InterventionHook/GenerationInterventionController/RunLog driving,
ported (logic) BYTE-IDENTICAL from `gate-contribution-factorial/steer_lib.py`
(read in full before writing this): same `erase_write`/`anchor_onward`
InterventionHook law/position, same `run_batch_fixed`/`run_rows` batching and
readback-instrumentation contract. Only the namespaced render env vars
differ (MARGIN_RENDER_MODEL/MARGIN_RENDER_REVISION instead of
GATEFACT_RENDER_MODEL/GATEFACT_RENDER_REVISION), carrying forward the same
RR/RR2/RR3/midband-heldout/factorial render-env fix.

Single dose per pass: every row in one `run_rows` call shares the same rung
(cell.yaml `ladder`: "every row in the population is dosed at every ladder
step" -- M1 has no gate, so unlike the factorial's arms this is simply "one
call per (family, rung)", not "one call per (family, arm, rung)").
"""
...
def load_model(model_name: str, revision: str):
    """Loads the generation model/tokenizer AND sets M4WK_RENDER_MODEL/
    M4WK_RENDER_REVISION (THIS experiment's own render.py env-var names --
    NOT M1's MARGIN_RENDER_MODEL/MARGIN_RENDER_REVISION, since this is a
    local copy of M1's steer_lib.py and must agree with M4-WK's own
    render.py) to the SAME model/revision, in the same place, so the two can
    never drift apart. Qwen3.5 requires transformers >= 5.x (run under
    /home/profsynapse/miniconda3/bin/python3, base conda, per
    midband-heldout's own documented deviation-with-cause)."""
    import torch
    from transformers import AutoModelForCausalLM, AutoTokenizer

    os.environ["M4WK_RENDER_MODEL"] = model_name
    os.environ["M4WK_RENDER_REVISION"] = revision or ""
...
def build_hook_and_controller(direction_vec, sigma: float):
    ...
    `ladder.write.law`/`ladder.write.position`)."""
...
def run_batch_fixed(
    ...
    every row in `prompts` shares the same rung. Termination rule is
    eos-anywhere, the RR/RR2/RR3/midband-heldout/factorial convention."""
    import torch
...
def run_rows(
    ...
    """Generic batched runner for ONE (family, rung) pass (every row passed
    in shares `mode`/`strength`): renders, generates, grades (full sub-grade
    dict via gen_lib.grade_row, data-exhaust rule), RunLog-records each row
    keyed by row_key. Resumable: already-recorded row_keys are skipped.
    ...
    rung hard-aborts before the rest of its rows are spent -- the callback
    is expected to raise SystemExit on failure; `run_rows` itself does not
