"""Generation-side grading helpers for margin-mapping (M1).

Byte-identical LOGIC to `gate-contribution-factorial/gen_lib.py` (read in
full before writing this; only this docstring differs): `final_rate_rule` is
...
(AMENDMENT.md: "Well-formedness is graded by the unchanged JSON parse rule
and is not adjudicated"), and `grade_row` merges the same two sub-grades
(clean_tighten + detector_v2) M1's `well_formed`/`collapse_dose` deliverables
are computed from directly.
"""
...
def grade_clean_tighten(raw_text: str, terminated_naturally: bool) -> dict:
    """`well_formed` (plain JSON-parse check -- this is the field M1's
    ...
    sub-grade, ported verbatim (logic) from the factorial's gen_lib.py."""
    text = raw_text or ""
...
def grade_row(text: str, terminated_naturally: bool, aliases: Optional[list[str]]) -> dict:
    ...
    is the `detector_v2_refused` half of M1's `refused_final_rule`
