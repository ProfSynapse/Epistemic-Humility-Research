...
repo-root `bin/sync_skills.py --check` and asserts exit 0, making "the trees agree" a
...
# to the repo root that owns bin/sync_skills.py. We don't hardcode the depth: search
# upward for the first ancestor containing bin/sync_skills.py so the test works
...
SYNC_SCRIPT = REPO_ROOT / "bin" / "sync_skills.py"
...
def test_skill_trees_in_sync():
    """`bin/sync_skills.py --check` must exit 0 — no drift across the three trees.

    A non-zero exit means the canonical .skills/ source and a mirror have
    diverged; the fix is `python3 bin/sync_skills.py --write` (never a hand-edit to a
    ...
    assert result.returncode == 0, (
        ...
        f"source (run `python3 bin/sync_skills.py --write`):\n{result.stdout}{result.stderr}"
    )
...
def test_project_context_docs_in_sync():
    ...
    assert result.returncode == 0, (
        ...
        f"(run `python3 bin/sync_skills.py --write`):\n{result.stdout}{result.stderr}"
    )
