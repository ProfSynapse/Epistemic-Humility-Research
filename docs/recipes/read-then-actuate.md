# Recipe: read-then-actuate on an open-weight model

**What this is.** A procedure for taking an arbitrary open-weight instruction-tuned
model and (a) fitting a **known-unknown (KU) direction** in its residual stream,
(b) building a **KU readout gate** on that direction, and (c) attempting a
**boundary push (dosed write)** along a paired write direction to make the model
hedge on questions it would otherwise confabulate an answer to.
...
| Write-site selection | **UNSOLVED.** There is an empirical depth envelope and no mechanism. |
...
### 3.2 What is available: an empirical depth envelope
...
`relative_depth = layer_idx / num_hidden_layers`. Sourced decimals: llama hs17 =
**0.607**; gemma hs15/18/20/22/23/24 = 0.357/0.429/0.476/0.524/0.548/0.571.
(Relative depths for mistral hs15/hs16, Qwen3-4B hs23 and Qwen3.5-4B hs20 are
**NOT SOURCED** as decimals — they would be arithmetic from block counts.)

**How much this envelope is worth.** It is drawn around a handful of successes.
There is **no sourced example of a site inside the 0.60–0.64 band that failed** —
which sounds like support but is not: it means the band's edge has never been
probed. The nearest sourced failures are all *deeper* (llama hs20/22/23/26 on a
28-block model, rd ≥ 0.714, all failing G1). And **no family in this program has
been tested at single-block depth resolution.** Treat the envelope as the best
available prior and nothing more.
...
Sweep the band. Fit directions and calibrate a dose at **several** sites in
rd ≈ 0.35–0.61, run `A_lin` across all of them for the record, and let the dose
...
4. Fit `u_d`/`c_hat` at every candidate site in **rd 0.35–0.61**; require gate
   AUC ≥ 0.90; verify byte-identical refits. Run a **random-direction read
   control** (§2.4).
