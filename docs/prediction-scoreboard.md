...
> **AUDIT NOTICE (2026-07-20).** A programmatic audit of all 86
> `experiments/*/experiment.yaml` manifests found: (1) this file's header
> misstates the machine schema — the real fields are `prediction` (a
> YAML-string requiring a second parse) and `verdict`; no
> `predictions`/`outcome`/`scoreboard` keys exist anywhere; (2) this ledger
> silently tracks only the propensity-steering cluster — **23 additional
> two-sided, resolved amendments** (the training/readout track: J through Z,
> AA-AF, AM) have recorded orchestrator+user predictions and verdicts but no
> ledger row; (3) the AK row above was factually stale. A backfill
> adjudication worksheet is in preparation; until it lands, the running
> tally below covers ONLY the rows in the table above. The durable fix
> (machine-readable `scoreboard:` blocks written at resolve, a checked-in
> tally script, and a merge-githook consistency check) is queued as a
> process change.

Running tally (table rows only, see audit notice): **user 4 – orchestrator 5 – ties 2** (AG predates the
