...
|---|---|---|---|---|
| Two-signal probe directions | `professorsynapse/eh-probe-directions` | staged — awaiting user content review | `experiment/phase1/probe/steering/directions/` (Amendment AA fits, seed 20260630) | 4 families x gate/dial: per-layer unit-normed directions (safetensors) + fit metadata JSON + card + `PROVENANCE.json`. ~100 KB. License: Apache-2.0 (probe weights over our own activations). |
| Readout row surfaces | `professorsynapse/eh-readout-rows` | staged — awaiting user content review | per-folder source paths recorded in staged `PROVENANCE.json`; sources under `experiment/phase1/probe/` | 31 folders / 79,015 rows / ~62 MB: S/T/U/W/X/Z/SR/P generation surfaces (question, answer, grade — NO hidden states) + frozen probe-pool row layers. License pass done per source (SelfAware Apache-2.0, KUQ MIT, PopQA MIT companion, TriviaQA research-use, flagged on the card). No OpenMOSS/Cheng IDK content; smokes excluded. |
