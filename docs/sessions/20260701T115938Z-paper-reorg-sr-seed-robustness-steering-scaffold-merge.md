...
- id: 003-result
  ...
  evidence:
  - archive/experiment/phase1/probe/sr_logs/PROGRESS.log
  run_ids: []
  commands: []
  decisions: []
  next_steps:
...
- evidence:
  - `archive/experiment/phase1/probe/sr_logs/PROGRESS.log`
- next steps:
  - Let the queue finish; fill AMENDMENT-SR section 7 per-seed tables + seed-stability roll-up + per-seed veto majority + locked verdict; refresh experiment note; open the SR PR.
...
- decisions:
  - Record Gemma as RE-RUN PENDING, not INELIGIBLE (mkdir error is not a pre-reg blocker). DEFER the SR verdict: strict per-seed clause (c) at seed 701 hinges on Gemma-701 (2/4 without it; seeds 702/703 already 3/4). section 7 + experiment note updated and committed (f7cc4ad6).
- next steps:
...
- evidence:
  ...
  - `archive/experiment/phase1/probe/sr_logs/PROGRESS.log`
- next steps:
  - When seed 703 lands: fill Gemma rows in AMENDMENT-SR section 7, compute the locked verdict, finalize paper3 section 4.8 + section 4.7/abstract, commit, open the SR PR, update the amendment-sr memory.
