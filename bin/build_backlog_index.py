...
Scans experiments/*/experiment.yaml records with a legacy amendment label, reads
the co-located AMENDMENT.md ``Status:`` line and any ``## 8. Result`` /
``## Result`` / ``VERDICT:`` verdict, classifies the status into a small fixed
vocabulary, and rewrites the fenced GENERATED block in TODO.md. Everything
outside the fence (the hand-curated prioritized backlog and any prose) is
preserved verbatim, so the file is safe to regenerate.

Deterministic and idempotent. See docs/backlog/PLAN.md for design.
...
def legacy_label(manifest: dict, fallback_title: str) -> str | None:
    legacy = manifest.get("legacy") or {}
    if isinstance(legacy, dict) and legacy.get("label"):
        return str(legacy["label"]).upper()
    title = str(manifest.get("title") or fallback_title)
    m = re.match(r"Amendment\s+([A-Z]{1,2})\b", title, re.I)
...
def collect() -> list[dict]:
    rows = []
    for manifest_path in sorted(EXPERIMENTS_DIR.glob("*/experiment.yaml")):
        manifest = yaml.safe_load(manifest_path.read_text(encoding="utf-8")) or {}
        if not isinstance(manifest, dict):
            continue
        amendment_path = manifest_path.parent / "AMENDMENT.md"
        if not amendment_path.is_file():
            continue
        letter = legacy_label(manifest, manifest_path.parent.name)
        if not letter:
            continue
        text = amendment_path.read_text(encoding="utf-8")
        status_line = parse_status_line(text)
        legacy = manifest.get("legacy") or {}
        legacy_path = legacy.get("path") if isinstance(legacy, dict) else ""
        doc_label = Path(str(legacy_path)).name if legacy_path else "AMENDMENT.md"
        rows.append(
            {
                "letter": letter,
                "title": extract_title(text, str(manifest.get("title") or manifest_path.parent.name)),
...
def render_block(
    ...
    lines = [
        ...
        "each migrated `experiments/<slug>/AMENDMENT.md` with a legacy amendment "
        "label. The `WT` column is derived "
