...
Notes live in `experiment/notes/<slug>.md` and are first-class knowledge-graph
...
2. Governance = `experiment/protocol/PROTOCOL.md` + `amendment-governance.md`.
...
(starts with one of `experiment/ library/ meta-analysis/ bin/ tools/ docs/
.agents/ .skills/ .claude/ .github/`) must exist on disk. This catches rot when a
...
`experiment/notes/README.md` is the auto-generated registry (status | phase | gap
