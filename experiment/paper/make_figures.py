...
randomness, no network, CPU only. Regenerate with:
    ...
    python3 experiment/paper/make_figures.py
...
def fig1_cross_family():
    ...
    fig.tight_layout()
    fig.savefig(OUT / "fig1_cross_family_readout.png", bbox_inches="tight")
    plt.close(fig)
...
def fig2_dial_distribution():
    ...
    fig.tight_layout()
    fig.savefig(OUT / "fig2_dial_distribution.png", bbox_inches="tight")
    plt.close(fig)
...
def fig3_fragile_axis():
    ...
    fig.suptitle("The gate and dial are stable; the veto is the fragile axis — in both directions",
                 fontsize=12, y=1.02)
    fig.tight_layout()
    fig.savefig(OUT / "fig3_fragile_axis.png", bbox_inches="tight")
    plt.close(fig)
...
def fig4_post_beats_pre():
    ...
    fig.suptitle("Per-answer correctness reads best AFTER the answer, peaking mid-network",
                 fontsize=12, y=1.02)
    fig.tight_layout()
    fig.savefig(OUT / "fig4_post_beats_pre.png", bbox_inches="tight")
    plt.close(fig)
...
def fig5_training_sharpens():
    ...
    fig.suptitle("Training does not create the trust signal — it sharpens the veto (Qwen3-4B)",
                 fontsize=12, y=1.02)
    fig.tight_layout()
    fig.savefig(OUT / "fig5_training_sharpens.png", bbox_inches="tight")
    plt.close(fig)
...
def fig6_pipeline():
    ...
    fig.tight_layout()
    fig.savefig(OUT / "fig6_pipeline.png", bbox_inches="tight")
    plt.close(fig)
