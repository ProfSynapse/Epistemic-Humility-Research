"""Build reproducible Paper 2 result tables from local Phase 1 eval artifacts.
...
DEFAULT_OUT = REPO / "experiment" / "paper" / "analysis"
DEFAULT_FIGURES = REPO / "experiment" / "paper" / "figures"
AMENDMENT_B_REPORT = (
    REPO / "experiment" / "phase1" / "eval" / "analysis" / "amendment_b_sequential_results_report.md"
)
        ...
        rows.append(
            {
                ...
                "source_metrics": "local Phase 1 stated-confidence metrics.json files",
            }
        )
    return rows
            ...
            for line in fh:
                ...
                if key not in buckets:
                    buckets[key] = {
                        ...
                        "known_sq_error": 0.0,
                        "high_conf": 0,
                        "source_rows": "local Phase 1 stated-confidence scored_rows.jsonl files",
                    }
                acc = buckets[key]
...
def write_figures(
    ...
    write_tradeoff_figure(figures_dir / "fig-01-cold-start-tradeoff.svg", seed_rows)
    write_metric_bar_figure(figures_dir / "fig-02-selfaware-metrics.svg", summary_rows)
    write_transition_figure(figures_dir / "fig-03-paired-transitions.svg", transition_rows)
    write_amendment_tradeoff_figure(figures_dir / "fig-04-sft-warmed-tradeoff.svg", amendment_rows)
    write_stated_confidence_figure(figures_dir / "fig-05-stated-confidence.svg", confidence_rows)
    write_tradeoff_png(figures_dir / "fig-01-cold-start-tradeoff.png", seed_rows)
    write_metric_bar_png(figures_dir / "fig-02-selfaware-metrics.png", summary_rows)
    write_transition_png(figures_dir / "fig-03-paired-transitions.png", transition_rows)
    write_amendment_tradeoff_png(figures_dir / "fig-04-sft-warmed-tradeoff.png", amendment_rows)
    write_stated_confidence_png(figures_dir / "fig-05-stated-confidence.png", confidence_rows)
    write_confidence_alignment_png(figures_dir / "fig-06-confidence-alignment.png", alignment_rows)
...
def write_report(
    ...
    lines = [
        ...
        "",
        "Generated by `experiment/paper/scripts/build_paper2_results.py` from local Phase 1 eval artifacts.",
        "",
        ...
        *_markdown_table(
        ...
        "## Generated Figures",
        "",
        "- `experiment/paper/figures/fig-01-cold-start-tradeoff.svg`",
        "- `experiment/paper/figures/fig-02-selfaware-metrics.svg`",
        "- `experiment/paper/figures/fig-03-paired-transitions.svg`",
        "- `experiment/paper/figures/fig-04-sft-warmed-tradeoff.svg`",
        "- `experiment/paper/figures/fig-05-stated-confidence.svg`",
        "- `experiment/paper/figures/fig-06-confidence-alignment.png`",
        "",
    ]
...
def main() -> int:
    ...
    write_report(
        out_dir / "paper2_results_analysis.md",
        seed_rows,
        summary_rows,
        transition_rows,
        amendment_rows,
