...
_MAX_OPERATOR_SNAPSHOT_BYTES = 64 * 1024 * 1024
...
def read_validated_operator_snapshot(
    descriptor: int, *, expected_identity: tuple[int, ...]
) -> sqlite3.Connection:
    """Deserialize one bounded, quiescent database image from a secured fd.

    The caller rejects WAL/SHM state and revalidates the pathname and private
    ancestors. This helper never opens, checkpoints, repairs, or migrates them.
    """
    ...
    if (
        ...
        or not 0 < before.st_size <= _MAX_OPERATOR_SNAPSHOT_BYTES
    ):
        raise ValueError("training ledger snapshot is unavailable")
    chunks: list[bytes] = []
    offset = 0
    while offset < before.st_size:
        chunk = os.pread(descriptor, min(1024 * 1024, before.st_size - offset), offset)
        if not chunk:
            raise ValueError("training ledger snapshot changed")
        chunks.append(chunk)
        offset += len(chunk)
    ...
    if identity != after_identity or os.pread(descriptor, 1, before.st_size):
        raise ValueError("training ledger snapshot changed")
    image = bytearray(b"".join(chunks))
    if len(image) < 100 or image[:16] != b"SQLite format 3\x00":
        raise ValueError("training ledger schema is invalid")
    # A cleanly closed WAL database retains WAL read/write version bytes even
    # though all durable pages are in this main-file image. Detach that image
    # from sidecar semantics before deserializing it; source bytes are untouched.
    if image[18:20] == b"\x02\x02":
        image[18:20] = b"\x01\x01"
    elif image[18:20] != b"\x01\x01":
        raise ValueError("training ledger schema is invalid")
    ...
    try:
        ...
        validate_production_v2_schema(connection)
        return connection
    except Exception:
        connection.close()
        raise ValueError("training ledger schema is invalid") from None


def validate_production_v2_schema(connection: sqlite3.Connection) -> None:
    """Require the exact v2 schema generated by this repository's trusted DDL."""
    names = (*_BASE_TABLE_COLUMNS, *_DOCKER_TABLE_COLUMNS)
    ...
    if object_types - {"table", "index"}:
        raise ValueError("training ledger schema is invalid")
    expected = sqlite3.connect(":memory:")
    expected.row_factory = sqlite3.Row
    try:
        SqliteTrainingRepository._create_base_schema(expected)
        SqliteTrainingRepository._create_docker_schema(expected)
        expected.execute(
            "INSERT INTO schema_meta(key, value) VALUES('schema_version', ?)",
            (_SCHEMA_VERSION,),
        )
        expected_objects = tuple(expected.execute(
            f"""SELECT type, name, tbl_name, sql FROM sqlite_master
                 WHERE tbl_name IN ({placeholders}) AND type IN ('table', 'index')
                 ORDER BY type, name""",
            names,
        ).fetchall())
    finally:
    ...
    tables = {
        str(row[0]) for row in connection.execute(
            "SELECT name FROM sqlite_master WHERE type='table'"
        ).fetchall() if row[0] != "sqlite_sequence"
    }
    required = frozenset(names)
    if tables - _COEXISTING_TABLES != required:
        raise ValueError("training ledger schema is invalid")
    observed_objects = tuple(connection.execute(
        f"""SELECT type, name, tbl_name, sql FROM sqlite_master
             WHERE tbl_name IN ({placeholders}) AND type IN ('table', 'index')
             ORDER BY type, name""",
        names,
    ).fetchall())
    if tuple(map(tuple, observed_objects)) != tuple(map(tuple, expected_objects)):
        raise ValueError("training ledger schema is invalid")
    metadata = connection.execute("SELECT key, value FROM schema_meta").fetchall()
    if [tuple(row) for row in metadata] != [("schema_version", _SCHEMA_VERSION)]:
        raise ValueError("training ledger schema is invalid")
