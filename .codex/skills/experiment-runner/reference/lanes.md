...
## Why two data-feeding strategies (the data-locality contract, arch section 9.2(b.1))
...
`/workspace/repo / local_file`). The research repo's `archive/experiment/phase1/data/` is
...
1. Stage the per-cell `<method>_train.jsonl` + `_dev.jsonl` from
   `archive/experiment/phase1/data/<model_tag>/` into the tuner's **already gitignored**
...
**Uniform dispatch (HANDLER EXTENSION, section 9.2 :994-1013, lead-ratified final).**
...
(that direct-trainer path was explicitly WITHDRAWN, section 9.2 :1175-1176).
...
locked training-regimen datasets are published publicly via the tuner's dataset-publishing
...
silent corruption of the seed sweep and beta panel (the HANDOFF section 5 failure mode).
...
contract (section 9.2 :994-1013 + (d) item 5, lead-ratified final). Three things must hold:
...
3. **LoRA flag parity** -- `train_dpo.py` AND `train_kto.py` accept every `--lora-*`
   ...
   parity because the recipe `lora:` block is the section 5.2 budget SSOT and the 8B
   recipes pin `r=64/Î±=128` vs the trainer `config.yaml` default `r=32/Î±=64` --
   ...
   was withdrawn (section 9.2 :1175-1176). The probe reads False against a pre-#34 tree
...
(section 9.2(d) item 5): a missing seed/beta sink makes every headline seed identical and
