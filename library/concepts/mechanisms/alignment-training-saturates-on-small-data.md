...
Saeidi et al. (2024, Figure 3) show that across all four alignment methods in Scenario 1, MT-Bench performance peaks within training sets of 1K--10K data points. The authors attribute the saturation to a division of labor: the SFT stage already instills the core behavioral patterns, leaving the preference stage to refine a model that has little residual room to improve with more paired examples. This mechanism is directly relevant to locked training-regimen experiment design: locked training-regimen runs do not require large preference datasets and may benefit from deliberately small budgets that avoid the degradation zone.
